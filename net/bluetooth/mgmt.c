...
#define SCAN_LE		0x01
#define SCAN_BR		0x02
/* FUJITSU:2013-06-18 FJ BLE add start */
#define SCAN_LE_ONLY	0x03
/* FUJITSU:2013-06-18 FJ BLE add end */
...
/* FUJITSU:2013-06-18 FJ BLE add start */
static int le_start_scan(struct sock *sk, u16 index)
{
	struct hci_cp_le_set_scan_enable le_cp = {1, 0};
	struct hci_dev *hdev;
	int err;
	...
	hdev = hci_dev_get(index);
	if (!hdev) {
		return cmd_status(sk, index, MGMT_OP_LE_START_SCAN, ENODEV);
	}
	...
	err = hci_send_cmd(hdev, HCI_OP_LE_SET_SCAN_ENABLE, sizeof(le_cp), &le_cp);

	if (err < 0) {
	} else if (lmp_le_capable(hdev)) {
		hdev->disco_state = SCAN_LE_ONLY;
		del_timer(&hdev->disco_le_timer);
		del_timer(&hdev->disco_timer);
	}
...
failed:
	if (err < 0) {
		err = cmd_status(sk, index, MGMT_OP_LE_START_SCAN, -err);
	} else {
		err = cmd_complete(sk, index, MGMT_OP_LE_START_SCAN, NULL, 0);
	}
...
static int le_stop_scan(struct sock *sk, u16 index)
{
	struct hci_cp_le_set_scan_enable le_cp = {0, 0};
	struct hci_dev *hdev;
//	struct pending_cmd *cmd;
	...
	hdev = hci_dev_get(index);
	if (!hdev) {
		return cmd_status(sk, index, MGMT_OP_LE_STOP_SCAN, ENODEV);
	}

	hci_dev_lock_bh(hdev);

	if (hdev->disco_state == SCAN_LE_ONLY) {
		hdev->disco_state = SCAN_IDLE;
	}
	...
	if (err < 0) {
		err = cmd_status(sk, index, MGMT_OP_LE_STOP_SCAN, -err);
	} else {
		err = cmd_complete(sk, index, MGMT_OP_LE_STOP_SCAN, NULL, 0);
	}
...
static int le_setup_scan_params(struct sock *sk, u16 index, unsigned char *data, u16 len)
{
	struct hci_cp_le_set_scan_parameters le_cp;
	struct hci_dev *hdev;
	struct mgmt_cp_le_setup_scan_params *cp = (void *)data;
	int err;

	if (len != sizeof(*cp)) {
		return cmd_status(sk, index, MGMT_OP_LE_SETUP_SCAN_PARAMS, EINVAL);
	}
	...
	memset(&le_cp, 0, sizeof(le_cp));
	le_cp.type = 0x01;
	le_cp.interval = cpu_to_le16(cp->scan_interval);
	le_cp.window = cpu_to_le16(cp->scan_window);
	le_cp.own_bdaddr_type = 0; /* Public address */
/* FUJITSU:2013-07-10 FJ BLE mod start */
#if 0
	le_cp.filter = 0;
#else
	le_cp.filter = cp->filter_policy;
#endif
/* FUJITSU:2013-07-10 FJ BLE mod end */

	err = hci_send_cmd(hdev, HCI_OP_LE_SET_SCAN_PARAMETERS, sizeof(le_cp), &le_cp);

	if (err < 0) {
		err = cmd_status(sk, index, MGMT_OP_LE_SETUP_SCAN_PARAMS, -err);
	} else {
		err = cmd_complete(sk, index, MGMT_OP_LE_SETUP_SCAN_PARAMS, NULL, 0);
	}
...
int mgmt_control(struct sock *sk, struct msghdr *msg, size_t msglen)
{
	...
	case MGMT_OP_LE_CANCEL_CREATE_CONN:
		...
		break;
/* FUJITSU:2013-06-18 FJ BLE add start */
	case MGMT_OP_LE_START_SCAN:
		err = le_start_scan(sk, index);
		break;
	case MGMT_OP_LE_STOP_SCAN:
		err = le_stop_scan(sk, index);
		break;
	case MGMT_OP_LE_SETUP_SCAN_PARAMS:
		err = le_setup_scan_params(sk, index, buf + sizeof(*hdr), len);
		break;
/* FUJITSU:2013-06-18 FJ BLE add end */
	default:
