...
/* FUJITSU:2012-10-25 FMP, PXP support  add start */
	clear_bit(HCI_LE_CONN_UPDATE_PEND, &conn->pend);
	if (test_and_clear_bit(HCI_LE_START_ENC_PEND, &conn->pend)) {
		u8 rand[8];
		__le16 ediv;
		...
		hci_le_start_enc(conn, ediv, rand, conn->stk);
	}
...
/* FUJITSU:2013-06-18 FJ BLE add end */
	...
	while (num_reports--) {
/* FUJITSU:2013-06-18 FJ BLE add start */
		rssi = (s8 *)&ev->data[ev->length];
/* FUJITSU:2013-06-18 FJ BLE add end */
/* FUJITSU:2013-06-18 FJ BLE mod start */
#if 0
		mgmt_device_found(hdev->id, &ev->bdaddr, ev->bdaddr_type,
				1, NULL, 0, ev->length, ev->data);
#else
		mgmt_device_found(hdev->id, &ev->bdaddr, ev->bdaddr_type,
				1, NULL, *rssi, ev->length, ev->data);
#endif
/* FUJITSU:2013-06-18 FJ BLE mod end */
