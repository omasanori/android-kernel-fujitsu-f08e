...
/* FUJITSU:2012-10-25 FMP, PXP support  add start */
		clear_bit(HCI_LE_START_ENC_PEND, &hcon->pend);
/* FUJITSU:2012-10-25 FMP, PXP support  add end */
...
/* FUJITSU:2012-10-25 FMP, PXP support  modification start */
#if 0
		hci_le_start_enc(hcon, ediv, rand, stk);
#else
		if (test_bit(HCI_LE_CONN_UPDATE_PEND, &hcon->pend)) {
			set_bit(HCI_LE_START_ENC_PEND, &hcon->pend);
			memcpy(hcon->stk, stk, 16);
		} else {
			hci_le_start_enc(hcon, ediv, rand, stk);
		}
#endif
/* FUJITSU:2012-10-25 FMP, PXP support  modification end */
...
static int smp_distribute_keys(struct l2cap_conn *conn, __u8 force)
{
	...
	if (hcon->out) {
		...
		del_timer(&hcon->smp_timer);
		clear_bit(HCI_CONN_ENCRYPT_PEND, &hcon->pend);
/* FUJITSU:2012-10-25 FMP, PXP support  start */
		clear_bit(HCI_LE_START_ENC_PEND, &hcon->pend);
/* FUJITSU:2012-10-25 FMP, PXP support  end */
		hci_conn_put(hcon);
...
/* FUJITSU:2012-10-25 FMP, PXP support  add start */
	clear_bit(HCI_LE_START_ENC_PEND, &conn->hcon->pend);
/* FUJITSU:2012-10-25 FMP, PXP support  add end */
