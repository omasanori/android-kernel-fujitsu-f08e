...
void l2cap_send_cmd(struct l2cap_conn *conn, u8 ident, u8 code, u16 len, void *data)
{
	...
	if (!skb)
		return;
	/* FUJITSU:2012-11-28 Connectivity BT mod start */
	if (conn->hcon == NULL || conn->hcon->hdev == NULL) {
		kfree_skb(skb);
		return;
	}
	/* FUJITSU:2012-11-28 Connectivity BT mod end   */
...
static int l2cap_parse_conf_rsp(struct sock *sk, void *rsp, int len, void *data, u16 *result)
{
	...
	/* FUJITSU:2012-12-12 Connectivity BT add start */
	rfc.txwin_size   = 0;
	/* FUJITSU:2012-12-12 Connectivity BT add end   */
