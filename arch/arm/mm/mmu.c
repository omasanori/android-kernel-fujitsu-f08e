...
void __init paging_init(struct machine_desc *mdesc)
{
	...
	{
		extern void ftadrv_bootmem_init(void);
		ftadrv_bootmem_init();
	}
...
int set_attributes_pmd(struct map_desc *md)
{
	unsigned long addr,addr_u,addr_m,length, end;
	phys_addr_t phys,phys_u,phys_m;
	const struct mem_type *type;
	pgd_t *pgd;
	pud_t *pud;
	pmd_t *pmd;
	unsigned long pgd_next,pud_next;
	pmd_t *p;		// flush addr


	type = &mem_types[md->type];

	/* Is an appointed logic address a section border(pmd)? */
	if(md->virtual & ~SECTION_MASK)
	{
		/* When it is not a section border */
		return(-1);
	}

//	addr = md->virtual & PAGE_MASK;
	addr = md->virtual & SECTION_MASK;		/*　section Mask */
	phys = __pfn_to_phys(md->pfn);
//	length = PAGE_ALIGN(md->length + (md->virtual & ~PAGE_MASK));
	length = ALIGN(md->length+(md->virtual & ~SECTION_MASK), SECTION_SIZE);
	/* PGD entry */
	pgd = pgd_offset_k(addr);
	end = addr + length;
	...
	do {
		...
		addr_u = addr;
		phys_u = phys;
		/* Loop PUD ARM PUD Nonimplement */
		do {
			pud_next = pud_addr_end(addr_u, pgd_next);
			pmd = pmd_offset(pud, addr_u);
			addr_m = addr_u;
			phys_m = phys_u;
			p = pmd;
			if (addr & SECTION_SIZE)
			...
			/* Loop PMD 1G */
			do {
				printk(KERN_NOTICE "p:%x:n:0x%08lx:o:0x%08lx ",phys_m,
					(long)__pmd(phys_m|type->prot_sect),
					(long)pmd_val(*pmd));
				*pmd = __pmd(phys_m | type->prot_sect);
				phys_m += SECTION_SIZE;
			} while (pmd++, addr_m += SECTION_SIZE, addr_m != pud_next);
			flush_pmd_entry(p);
			phys_u += pud_next - addr_u;
		} while (pud++, addr_u = pud_next, addr_u != pgd_next);
		phys += pgd_next - addr;
		addr = pgd_next;
	} while (pgd++, addr != end);
...
/*
 * It is stored away extable by a section of RODATA
 */

//int __init set_ro_rodata_attributes(void)
int set_ro_rodata_attributes(void)
{
	struct map_desc map;
	int		rc=0;
	map.pfn = __phys_to_pfn(__pa(__start_rodata));
	...
	map.length = __init_begin - __start_rodata;
	map.type = MT_MEMORY_R;
	rc = set_attributes_pmd(&map);
	return(rc);
}
//EXPORT_SYMBOL(set_ro_rodata_attributes);

/*
 * It is set init section at the time of init section liberation 
 * by attribute RW
 */
//int __init set_rw_initsection_attribute(void)
int set_rw_initsection_attribute(void)
{
	struct map_desc map;
	int		rc=0;
...
//EXPORT_SYMBOL(set_rw_initsection_attributes);
#endif
