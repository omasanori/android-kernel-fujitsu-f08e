...
/*FUJITSU:2013-03-15 RAMDUMP ADD START */
#if 0
static int download_mode = 1;
#else
static int download_mode = 0;
#endif
/*FUJITSU:2013-03-15 RAMDUMP ADD END */
...
/* FUJITSU:2013-01-28 shutdown/reboot fail safe add start */
	...
	while (1)
		;
/* FUJITSU:2013-01-28 shutdown/reboot fail safe add end */
	return;
}
...
{
/* FUJITSU:2013-01-28 shutdown/reboot fail safe del start */
#if 0
	int rc;
#endif
/* FUJITSU:2013-01-28 shutdown/reboot fail safe del end */

	pr_err("PMIC Initiated shutdown %s cpu=%d\n", __func__,
						smp_processor_id());
	if (smp_processor_id() == 0) {
/* FUJITSU:2013-01-28 shutdown/reboot fail safe mod start */
#if 0
		/*
...
#else
		/*
		 * We exchange the power off that is movement of the default of 
		 * the power key push and hold with hard reset.
		 * If we wait for ten seconds, and PMIC does not perform hard reset,
		 * We give it an order to start power off for PMIC.
		 * Because We want to do cold boot, We do not perform the WDT time-out.
		 */
		printk(KERN_INFO "Waiting Hard Reset cpu=%d\n", smp_processor_id());
		mdelay(10000);
		printk(KERN_INFO "Fail safe restart=%d\n", smp_processor_id());
		pm8xxx_reset_pwr_off(1);
#endif
/* FUJITSU:2013-01-28 shutdown/reboot fail safe mod end */
	}
...
/* FUJITSU:2012-12-27 BootReason add start */
...
/* FUJITSU:2012-12-27 BootReason add end   */
		} else {
