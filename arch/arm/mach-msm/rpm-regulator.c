...
void rmp_vreg_dump_info(void)
{
	...
	if(!config || !config->vregs) {
		printk(KERN_INFO ",DRPM,ERR,ARG1\n");
		return;
	}

	printk(KERN_INFO "DRPM, name, is_enabled, voltage\n");
	for (i = RPM_VREG_ID_PM8921_L1; i <= RPM_VREG_ID_PM8921_NCP; i++){
		vreg = &config->vregs[i];
		if(!vreg) {
			printk(KERN_INFO "DRPM,%d,ERR,vreg=NULL\n", i);
			return;
		}else{
			local_rmp_vreg_dump_info(vreg->rdesc.name);
		}
	}
	local_rmp_vreg_dump_info("8821_s0");
	local_rmp_vreg_dump_info("8821_s1");
}
