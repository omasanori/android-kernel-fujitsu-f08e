...
/* FUJITSU:2013-01-28 watchdog bite time config add start */
#define WDT_BITE_DIF	10	/* bite 10sec after barked */
/* FUJITSU:2013-01-28 watchdog bite time config add end */
...
static void pet_watchdog_work(struct work_struct *work)
{
	...
	/* FUJITSU:2013-01-28 watchdog log add start */
	pr_info("***** pet_watchdog *****\n");
	/* FUJITSU:2013-01-28 watchdog log add end */
	if (enable)
...
/* FUJITSU:2013-01-28 watchdog bite time config mod start */
#if 1
	__raw_writel(timeout + WDT_BITE_DIF*WDT_HZ, msm_wdt_base + WDT_BITE_TIME);
#else
