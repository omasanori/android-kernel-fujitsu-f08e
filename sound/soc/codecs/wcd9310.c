...
#include "mc_yamaha/mc_asoc_priv.h"
extern int ymu_control(int scene, int param);
extern int ymu_volume(int amp, int vol);
...
/* FUJITSU Audio:20120-08-07 JBporting start */
#undef  DBG_PRINTK
#define DBG_PRINTK(fmt,...)  pr_info("%s(%d):%s " fmt "\n", __FILE__, __LINE__,__FUNCTION__, ##__VA_ARGS__)
/* FUJITSU Audio:20120-08-07 JBporting end */
...
/* ymu Output Path */
static const char	*ymu_output_path_param_text[] = {
	"NONE", "SP", "RC", "HP_L", "HP_R", "WM0010_USE", "REMOVE", "TEST", "LINEOUT"
};

static const struct soc_enum	ymu_output_path_param_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ymu_output_path_param_text), ymu_output_path_param_text);

static int ymu_output_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	DBG_PRINTK("start");
	return 0;
...
static int ymu_output_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int		err = 0;
	...
	case 0:		//"NONE"
		DBG_PRINTK("NONE");
		err = ymu_control(YMU_OUTPUT_OFF, 0);
		break;
	case 1:		//"SP"
		DBG_PRINTK("SP");
		err = ymu_control(YMU_SPEAKER, 0);
		break;
	case 2:		// "RC"
		DBG_PRINTK("RC");
		err = ymu_control(YMU_RECEIVER, 0);
		break;
	case 3:		// "HP_L"
		DBG_PRINTK("HP_L");
		err = ymu_control(YMU_EARPHONE_L, 0);
		break;
	case 4:		// "HP_R",
		DBG_PRINTK("HP_R");
		err = ymu_control(YMU_EARPHONE_R, 0);
		break;
	case 5:		// "WM0010_ON"
		DBG_PRINTK("WM0010_USE");
		err = ymu_control(YMU_OUTPUT_USE_WM0010, 0);
		break;
	case 6:		// "REMOVE"
		DBG_PRINTK("REMOVE");
		err = ymu_control(YMU_OUTPUT_OFF, 0);
	...
	case 7:		// "TEST"
		DBG_PRINTK("TEST");
		err = ymu_control(YMU_DEGITAL_OUTPUT, 0);
		break;
	case 8:		// "LINEOUT"
		DBG_PRINTK("LINEOUT");
		err = ymu_control(YMU_LINEOUT, 0);
		break;
	default:
		break;
	}
...
static const char	*ymu_input_path_param_text[] = {
	"NONE", "MainMIC", "SubMIC", "EarPhoneMIC","WM0010_USE", "REMOVE", "GainDefaultSet"
};
/* QNET Audio:2012-02-14 YAMAHA default volume set end */

static const struct soc_enum	ymu_input_path_param_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ymu_input_path_param_text), ymu_input_path_param_text);

static int ymu_input_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	DBG_PRINTK("start");
	return 0;
...
static int ymu_input_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int		err = 0;
...
//	DBG_PRINTK("start");
	switch(ucontrol->value.integer.value[0]) {
	case 0:		//"NONE"
		err = ymu_control(YMU_INPUT_OFF, 0);
		break;
	case 1:		//MainMIC"
		err = ymu_control(YMU_MIC_MAIN, 0);
		break;
	case 2:		//SubMIC"
		err = ymu_control(YMU_MIC_SUB, 0);
		break;
	case 3:		//EarPhoneMIC"
		err = ymu_control(YMU_MIC_EARPHONE, 0);
		break;
	case 4:		//"WM0010_ON"
		err = ymu_control(YMU_INPUT_USE_WM0010, 0);
		break;
	case 5:		//"REMOVE"
		err = ymu_control(YMU_INPUT_OFF, 0);
		break;
/* QNET Audio:2012-02-14 YAMAHA default volume set start */
	case 6:		// "GainDefaultSet"
		err = ymu_control(YMU_GAIN_DEFAULT_SET, 0);
		break;
/* QNET Audio:2012-02-14 YAMAHA default volume set start */
	default:
		break;
...
/* ymu Control */
static const char	*ymu_control_param_text[] = {
/* QNET Audio:2012-02-29 YAMAHA MUTE start */
	 "Circuit_All_OFF", "Circuit_Apply","MUTE", "ForceMuteON", "ForceMuteOFF"
/* QNET Audio:2012-02-29 YAMAHA MUTE end */
/* QNET Audio:2012-07-04 YAMAHA STREAM_ENFORCED start */
     , "StreamEnforcedON", "StreamEnforcedOFF"
...
static const struct soc_enum	ymu_control_param_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ymu_control_param_text), ymu_control_param_text);

static int ymu_control_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	DBG_PRINTK("start");
	return 0;
...
static int ymu_control_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int		err = 0;
	...
	DBG_PRINTK("start");
	switch(ucontrol->value.integer.value[0]) {
	case 0:		//"Circuit_All_OFF"
		err = ymu_control(YMU_CIRCUIT_ALLOFF, 0);
		break;
	case 1:		//"Circuit_Apply"
		err = ymu_control(YMU_CIRCUIT_SET, 0);
		break;
/* QNET Audio:2012-02-29 YAMAHA MUTE start */
	case 2:		// "MUTE"
		err = ymu_control(YMU_VOLUME_MUTE, 0);
		break;
/* QNET Audio:2012-02-29 YAMAHA MUTE end */
/* QNET Audio:2012-04-10 11_2nd Feat start */
	case 3: //ForceMute on
		err = ymu_control(YMU_VOLUME_MUTE, 1);
		break;
	case 4: //ForceMute off
		err = ymu_control(YMU_VOLUME_MUTE, 2);
		break;
/* QNET Audio:2012-04-10 11_2nd Feat   end */
/* QNET Audio:2012-07-04 YAMAHA STREAM_ENFORCED start */
	case 5:
		//StreamEnforcedON
		err = ymu_control(YMU_STREAM_ENFORCED,1);
		break;
	case 6:
		//StreamEnforcedOFF
		err = ymu_control(YMU_STREAM_ENFORCED,0);
		break;
/* QNET Audio:2012-07-04 YAMAHA STREAM_ENFORCED end */
/* FUJITSU nab audio: VOIP ADD-S */
/* Noah for 8k/16k sample rate start */
	case 7: //8K
		err = ymu_control(YMU_FS_SWITCH, 0);
		break;
	case 8: //16K
		err = ymu_control(YMU_FS_SWITCH, 1);
		break;
/* Noah for 8k/16k sample rate end */
/* FUJITSU nab audio: VOIP ADD-E */
	default:
...
/* MACH1 Volume */
static int volume_type = -1; // volume type

static const char	*ymu_volume_type_param_text[] = {
	"LIVOL_L", "LIVOL_R", "MIC1VOL", "MIC2VOL", "MIC3VOL", // Category A
	"AD0_ANA_L", "AD0_ANA_R",                              // Category B
	"HPVOL_L", "HPVOL_R", "SPVOL_L", "SPVOL_R", "RCVOL",   // Category C
	"LO1VOL_L", "LO1VOL_R", "LO2VOL_L", "LO2VOL_R",        // Category D
	"MIC1_GAIN", "MIC2_GAIN", "MIC3_GAIN",                 // Category E
	"HP_GAIN",                                             // Category F
};

static const struct soc_enum	ymu_volume_type_param_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ymu_volume_type_param_text), ymu_volume_type_param_text);

static int ymu_volume_type_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	DBG_PRINTK("start");
	return 0;
...
static int ymu_volume_type_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	DBG_PRINTK("start");

	/* keep volume type */
	volume_type = ucontrol->value.integer.value[0];

	pr_info("WCD: %s(%d) %s[volume]\n", __FILE__, __LINE__,__FUNCTION__);
	return 0;
}

static const char	*ymu_volume_value_param_text[] = {
...
static const struct soc_enum	ymu_volume_value_param_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ymu_volume_value_param_text), ymu_volume_value_param_text);

static int ymu_volume_value_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	DBG_PRINTK("start");
	return 0;
...
static int ymu_volume_value_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int		volume = -1000;
	int		value  = 0;
	...
	err = ymu_volume(volume_type, volume);
...
/* QNET Audio:2012-01-17 Basic Feat start */
	/* Output Path */
	SOC_ENUM_EXT("MACH1 Output", ymu_output_path_param_enum,
		ymu_output_get, ymu_output_set),
	/* Input Path */
	SOC_ENUM_EXT("MACH1 Input", ymu_input_path_param_enum,
		ymu_input_get, ymu_input_set),
	/* Control */
	SOC_ENUM_EXT("MACH1 Control", ymu_control_param_enum,
		ymu_control_get, ymu_control_set),
	/* Volume Type */
	SOC_ENUM_EXT("MACH1 Volume Type", ymu_volume_type_param_enum,
		ymu_volume_type_get, ymu_volume_type_put),
	/* Volume Value */
	SOC_ENUM_EXT("MACH1 Volume Value", ymu_volume_value_param_enum,
		ymu_volume_value_get, ymu_volume_value_put),
/* QNET Audio:2012-01-17 Basic Feat   end */
};
