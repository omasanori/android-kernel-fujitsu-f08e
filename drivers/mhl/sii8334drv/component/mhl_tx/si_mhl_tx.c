...
bool_t retVal;
	...
	if (pReq->payload_u.msgData[0] != MHL_MSC_MSG_RCPK) {
	    ...
	    CBUS_DEBUG_PRINT(("MhlTx:%d PutNextCBusTransaction %02X MSC_MSG_RCPK depth:%d head: %d tail:%d\n"
	                ,iLine
	                ,(int)pReq->command
	                ,(int)QUEUE_DEPTH(CBusQueue)
	                ,(int)CBusQueue.header.head
...
bool_t retVal;
	...
	if (pReq->payload_u.msgData[0] != MHL_MSC_MSG_RCPK) {
	    ...
	    CBUS_DEBUG_PRINT(("MhlTx:%d: PutPriorityCBusTransaction %02X MSC_MSG_RCPK depth:%d head: %d tail:%d\n"
	                ,iLine
	                ,(int)pReq->command
	                ,(int)QUEUE_DEPTH(CBusQueue)
	                ,(int)CBusQueue.header.head
...
static bool_t SiiMhlTxClrDCapRdy( void ) __attribute__ ((used));
static	bool_t 		SiiMhlTxRapkSend( void );
static bool_t SiiMhlTxSendLinkMode(void) __attribute__ ((used));
...
//				fj_SiiDebugPrint("Received RCP key code:0x%02x\n", mhlTxConfig.mscMsgData);
				if(MHL_LOGICAL_DEVICE_MAP & rcpSupportTable [mhlTxConfig.mscMsgData & 0x7F] )
				{
//					fj_SiiDebugPrint("RCP key code(0x%02x) is notified to application\n", mhlTxConfig.mscMsgData);
					AppNotifyMhlEvent(MHL_TX_EVENT_RCP_RECEIVED, mhlTxConfig.mscMsgData);
				}
				else
				{
//					fj_SiiDebugPrint("RCP key code(0x%02x) is not supported\n", mhlTxConfig.mscMsgData);
