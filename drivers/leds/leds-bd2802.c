...
#ifdef CONFIG_LEDS_BD2802_DEBUG

/* FUJITSU:2013-02-18 BD2802GU add start */
#define bd2802_debug_printk(format, arg...)
#define bd2802_debug2_printk(format, arg...) \
  printk("%s" format, KERN_DEBUG, ##arg)
/* FUJITSU:2013-02-18 BD2802GU add end */

#else /* CONFIG_LEDS_BD2802_DEBUG */

#define bd2802_debug_printk(format, arg...)
#define bd2802_debug2_printk(format, arg...)

#endif 	/* CONFIG_LEDS_BD2802_DEBUG */
...
{
/* FUJITSU:2013-01-23 BD2802GU mod start */
/*	int ret = i2c_smbus_write_byte_data(client, reg, val); */
	int ret;
	int	retry_count = 0;	/* FUJITSU:2013-02-18 BD2802GU add  */

	bd2802_debug2_printk("[%s] %x:%x \n",__func__,reg,val);

	/* FUJITSU:2013-02-18 BD2802GU add start*/
	for (;;) {
		ret = i2c_smbus_write_byte_data(client, reg, val);
/* FUJITSU:2013-01-23 BD2802GU mod end */
		if (ret >= 0)
			return 0;
		bd2802_debug2_printk("[%s] %x:%x err %d retry %d\n",__func__,reg,val,ret,retry_count);
		if (unlikely(retry_count >= 2)) {
			dev_err(&client->dev, "%s: reg 0x%x, val 0x%x, err %d retry over\n",
...
/*		gpio_set_value(led->pdata->reset_gpio, 0); */
		gpio_set_value_cansleep(led->pdata->reset_gpio, 0);
/* FUJITSU:2013-01-23 BD2802GU mod end */
/* FUJITSU:2013-01-23 BD2802GU add start */
		bd2802_debug_printk("[%s] gpio OFF \n",__func__);
/* FUJITSU:2013-01-23 BD2802GU add end */
		return;
	}
...
{
/* FUJITSU:2013-01-23 BD2802GU add start */
	bd2802_debug_printk("[%s] \n",__func__);
/* FUJITSU:2013-01-23 BD2802GU add end */

/* FUJITSU:2013-01-23 BD2802GU mod start */
/*	gpio_set_value(led->pdata->reset_gpio, 1); */
	gpio_set_value_cansleep(led->pdata->reset_gpio, 1);
/* FUJITSU:2013-01-23 BD2802GU mod end */
	udelay(100);
...
/* FUJITSU:2013-01-23 BD2802GU add start */
	bd2802_debug_printk("[%s] led->adf_on = %d \n",__func__,led->adf_on);
/* FUJITSU:2013-01-23 BD2802GU add end */
...
static ssize_t bd2802_store_adv_conf(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
/* FUJITSU:2013-02-20 BD2802GU delete start 
	struct bd2802_led *led = i2c_get_clientdata(to_i2c_client(dev));
   FUJITSU:2013-02-20 BD2802GU delete end */
...
{									\
	...
	up_write(&led->rwsem);					\
/* FUJITSU:2013-01-23 BD2802GU add start */		\
	bd2802_debug_printk("[%s] value = %d \n",__func__,led->attr_name);	\
/* FUJITSU:2013-01-23 BD2802GU add end */		\
	return count;							\
}									\
...
{									\
	struct bd2802_led *led =					\
		container_of(led_cdev, struct bd2802_led, cdev_##name);	\
/* FUJITSU:2013-01-23 BD2802GU del start */		\
/*	if (*delay_on == 0 || *delay_off == 0) */	\
/*		return -EINVAL; */						\
/* FUJITSU:2013-01-23 BD2802GU del end */		\
	led->led_id = id;						\
	led->color = clr;						\
...
/* FUJITSU:2013-02-20 BD2802GU add start */
	bd2802_enable_adv_conf(led);
/* FUJITSU:2013-02-20 BD2802GU add end */

	
/* FUJITSU:2013-01-23 BD2802GU add start */
	bd2802_debug_printk("[%s] ret = %d \n",__func__,ret);
/* FUJITSU:2013-01-23 BD2802GU add end */
...
/* FUJITSU:2013-01-23 BD2802GU add start */
static void bd2802_init_state(struct bd2802_led *led)
{
	int i;

	for (i = 0; i < LED_NUM; i++) {
		led->led[i].r = BD2802_OFF;
		led->led[i].g = BD2802_OFF;
		led->led[i].b = BD2802_OFF;
	}
}
/* FUJITSU:2013-01-23 BD2802GU add end */
