...
/* FUJITSU:2013-05-28 HMW_TOUCH_CONFIG_CRC add start */
#include "fj_touch_noracs.h"
/* FUJITSU:2013-05-28 HMW_TOUCH_CONFIG_CRC add end */
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	/* Check for config CRC */
		...
		pr_info("[TPD] %s: img_crc:0x%04X new_crc:0x%04X\n", __func__, data->si->ttconfig.crc, cfg_crc_new);		/* FUJITSU:2013-05-28 HMW_TOUCH_CONFIG_CRC mod */

		/* Check if config CRC different. */
		if (cfg_crc_new != data->si->ttconfig.crc) {
			pr_info("[TPD] %s: Config CRC different, will upgrade\n", __func__);		/* FUJITSU:2013-05-28 HMW_TOUCH_CONFIG_CRC mod */
			return 1;
		}
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4_PLATFORM_TTCONFIG_UPGRADE
/* FUJITSU:2013-05-28 HMW_TOUCH_CONFIG_CRC add start */
	if (cy_nv_info.not_write_config_parm == CY_TOUCH_DBG_ON) {
		pr_info("[TPD] %s : No write configuration parameter\n", __func__);
	} else {
/* FUJITSU:2013-05-28 HMW_TOUCH_CONFIG_CRC add end */
		if (!upgrade_ttconfig_from_platform(ttsp))
			return;
	}	/* FUJITSU:2013-05-28 HMW_TOUCH_CONFIG_CRC add */
#endif
}
