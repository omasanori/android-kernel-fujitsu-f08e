...
#define CY_DDATA_PANEL_VENDER_ID		0
#define CY_DDATA_PROJECT_ID				2			/* FUJITSU:2013-05-28 HMW_TOUCH_FWVER add */
#define CY_DDATA_LCD_VENDER_ID			4
...
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add start */
	struct cyttsp4_touch_firmware_info fi;
	struct cyttsp4_touch_platform_info pi;
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add end */
/* FUJITSU:2013-05-01 HMW_TTDA_MODIFY_LOG add start */
...
/* FUJITSU:2013-05-17 HMW_TOUCH_RUNTIME_CONFIG add start */
	u8 press_setting_val;						/* current press settings (now) */
	u8 press_setting_req;						/* next press settings (req) */
	u8 charger_armor_setting_val;				/* current charger armor settings (now) */
	u8 charger_armor_setting_req;				/* next charger armor settings (req) */
...
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add start */
	u8		press_normalize_nv_read;
	u8		press_normalize_sts;
	u8		press_force_calib_sts;
...
/* FUJITSU:2013-05-07 HMW_TTDA_RUNTIME_CONFIG add start */
static bool cyttsp4_is_sleep(struct cyttsp4_core_data *cd);
/* FUJITSU:2013-05-07 HMW_TTDA_RUNTIME_CONFIG add end */
/* FUJITSU:2013-04-15 HMW_TTDA_IOCTL add start */
static int cyttsp4_core_sleep(struct cyttsp4_core_data *cd);
static int cyttsp4_core_wake(struct cyttsp4_core_data *cd);
/* FUJITSU:2013-04-15 HMW_TTDA_IOCTL add end */

/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add start */
#define		CY_ENABLE_NORMALIZE_RETRY		3			/* num of press normalization's retry */

static int cyttsp4_get_parameter(struct cyttsp4_core_data *cd, u8 param_id, u32 *param_value);
static int cyttsp4_set_parameter(struct cyttsp4_core_data *cd, u8 param_id, u8 param_size, u32 param_value);

static int _cyttsp4_core_set_press_normalize_mode(struct cyttsp4_core_data *cd, u8 normalize_mode);
static int _cyttsp4_core_get_press_normalize_mode(struct cyttsp4_core_data *cd, u8 *normalize_mode);

#define		CY_SET_NORMALIZATION_MODE		0x12	/* set normalization mode */
#define		CY_GET_NORMALIZATION_MODE		0x13	/* get normalization mode */
#define		CY_SET_NORMALIZATION_PARM		0x14	/* set normalization parm */

#define		CY_IC_NORMALIZE_PARM_SIZE		80		/* Normalization parm NV Item size.(Byte) */
#define		CY_IC_NORMALIZE_1PARM_SIZE		6		/* Valid data size.(Byte) */
#define		CY_IC_NORMALIZE_1BLOCK_SIZE		8		/* 1 block size.(Byte) */
#define		CY_IC_NORMALIZE_POINT_NUM		9		/* point num */

/*
 * macro function.
 */
#include <stdarg.h>
	#define		_cyttsp4_dbg(cd, lv, fmt,...) {\
					_cyttsp4_log(_COR_REF_DEV(cd), lv, fmt, ## __VA_ARGS__);	\
				}

#define		_COR_REF_DEV(cd)		(((struct cyttsp4_core_data *)(cd))->dev)
#define		_COR_REF_ADPT(cd)		(((struct cyttsp4_core_data *)(cd))->core->adap)

/* I2C R/W */
#define		_cyttsp4_core_i2c_read_________(cd, cmd, len, buf)		cyttsp4_adap_read(cd, cmd, buf, len)
#define		_cyttsp4_core_i2c_write_________(cd, cmd, len, buf)		cyttsp4_adap_write(cd, cmd, buf, len)



static const u8 cDefault_NormalizeParm[ CY_IC_NORMALIZE_PARM_SIZE + 1 ] =
{
	/*
	 * 1 Piont == 8 Byte
	 * X[13:8], X[7:0], Y[13:8], Y[7:0], PressureZ[15:8](*1), PressureZ[7:0](*1), Reserve[15:8](*2), Reserve[7:0](*2)
	 *   *1 : PressureZ or LeftPressureZ
	 *   *2 : Reserve   or RightPressureZ
	 */
	0x00, 0x1E, 0x00, 0x1E, 0x00, 0x2F, 0x00, 0x2F,		/* Point 1 */
	0x01, 0x0E, 0x00, 0x1E, 0x00, 0x4B ,0x00, 0x4B,		/* Point 2 */
	0x01, 0xFD, 0x00, 0x1E, 0x00, 0x2F, 0x00, 0x2F,		/* Point 3 */
	0x00, 0x1E, 0x01, 0xE0, 0x00, 0x98 ,0x00, 0x98,		/* Point 4 */
	0x01, 0x0E, 0x01, 0xE0, 0x00, 0xF3, 0x00, 0xF3,		/* Point 5 */
	0x01, 0xFD, 0x01, 0xE0, 0x00, 0x98 ,0x00, 0x98,		/* Point 6 */
	0x00, 0x1E, 0x03, 0xA1, 0x00, 0x40, 0x00, 0x40,		/* Point 7 */
	0x01, 0x0E, 0x03, 0xA1, 0x00, 0x5E ,0x00, 0x5E,		/* Point 8 */
	0x01, 0xFD, 0x03, 0xA1, 0x00, 0x40, 0x00, 0x40,		/* Point 9 */
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00,		/* Reserve */
};

static int _cyttsp4_core_runtime_debug( struct cyttsp4_core_data *cd, const char *func, u8 reg, u32 expect_val);
#define    _cyttsp4_core_show_sw_status( cd ) _cyttsp4_dlog(cd->dev, " %s: sleep=0x%02X, mode = 0x%02X.\n", __func__, cd->sleep_state, cd->mode);

/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add end */
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_FORCE_CALIB add start */
static int _cyttsp4_core_do_force_calib(struct cyttsp4_core_data *cd);
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_FORCE_CALIB add end */
/* FUJITSU:2013-05-28 HMW_TOUCH_MULTI_MODEL add start */
static inline int _cyttsp4_core_is_support(struct cyttsp4_core_data *cd, int check_type)
{
	/* int check_type : CY_CORE_SUPPORT_PRESS, CY_CORE_SUPPORT_HOVER... cyttsp4_core.h */
	int ret = 0;		/* 0 : not support, 1 : support */

	if (cd->pdata->board_is_support) {
		ret = cd->pdata->board_is_support(cd->pdata, check_type, cd->dev);
	} else {
		_cyttsp4_elog(cd->dev, "%s : ptr is NULL, flags=0x%04X.\n", __func__, check_type);
	}
...
/* FUJITSU:2013-06-18 HMW_TOUCH_WATCHDOG_TEST add start */
/* ## config ## */
#define PM8921_GPIO_PM_TO_SYS(pm_gpio)  ((pm_gpio) + NR_GPIO_IRQS)
/* Setting Parameter */
#define SET_RECOVERY_LED_OFF  0x00
#define SET_RECOVERY_LED_ON   0x01

static int cyttsp4_set_led(struct cyttsp4_core_data *cd, int set)
{
	pr_info("[TPD]:%s LED[set=%d]\n", __func__, set);

	if(cd->nv_info->recovery_led){
		if(set == SET_RECOVERY_LED_OFF){
			gpio_set_value(PM8921_GPIO_PM_TO_SYS(0), 0);
			gpio_set_value(PM8921_GPIO_PM_TO_SYS(1), 0);
			gpio_set_value(PM8921_GPIO_PM_TO_SYS(2), 0);
		} else {
			gpio_set_value(PM8921_GPIO_PM_TO_SYS(0), 1);
			gpio_set_value(PM8921_GPIO_PM_TO_SYS(1), 1);
			gpio_set_value(PM8921_GPIO_PM_TO_SYS(2), 1);
		}
	}

	return 0;
}
/* FUJITSU:2013-06-18 HMW_TOUCH_WATCHDOG_TEST add end */
...
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add start */
	cd->fi.firmware_version = (((si->si_ptrs.cydata->revctrl[4] << 24) & 0xFF000000) |
							   ((si->si_ptrs.cydata->revctrl[5] << 16) & 0xFF0000) |
							   ((si->si_ptrs.cydata->revctrl[6] << 8) & 0xFF00) |
							     si->si_ptrs.cydata->revctrl[7]);
	pr_info("[TPD_Sy] firmware_version= 0x%X\n", cd->fi.firmware_version);
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add end */

/* FUJITSU:2013-05-28 HMW_TOUCH_FWVER mod start */
#if 0
...
#endif
	_cyttsp4_buf_dump((u8 *)si->si_ptrs.cydata, si->si_ofs.cydata_size - mfgid_sz, "sysinfo_cydata", CY_LOGLV1);
	_cyttsp4_buf_dump((u8 *)si->si_ptrs.cydata->mfg_id, mfgid_sz, "sysinfo_cydata_mfgid", CY_LOGLV1);
/* FUJITSU:2013-05-28 HMW_TOUCH_FWVER mod end */
...
/* FUJITSU:2013-05-28 HMW_TOUCH_INFORMATION add start */
	_cyttsp4_dbg(cd, CY_LOGLV0, "[TPD] %s test_data[%02X %02X].\n", __func__, si->si_ptrs.test->post_codeh, si->si_ptrs.test->post_codel);
...
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add start */
		if (si->si_ptrs.ddata != NULL) {
			memcpy(&cd->fi.panel_maker_code, &((u8 *)si->si_ptrs.ddata)[CY_DDATA_PANEL_VENDER_ID], 1);
			memcpy(&cd->fi.project_id, &((u8 *)si->si_ptrs.ddata)[CY_DDATA_PROJECT_ID], 1);				/* FUJITSU:2013-05-28 HMW_TOUCH_FWVER add */
			memcpy(&cd->fi.lcd_maker_code, &((u8 *)si->si_ptrs.ddata)[CY_DDATA_LCD_VENDER_ID], 1);
		}
		pr_info("[TPD_Sy] panel_code = 0x%X, lcd_code = 0x%X\n", cd->fi.panel_maker_code, cd->fi.lcd_maker_code);
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add end */
	}
	return rc;
}
...
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add start */
static int cyttsp4_request_hardware_info_(struct cyttsp4_device *ttsp, struct cyttsp4_touch_hardware_info *hi)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
	int rc = 0;
	...
	if(hi != NULL){
		hi->firmware_version = cd->fi.firmware_version;
		hi->config_crc       = cd->fi.config_crc;
		hi->panel_maker_code = cd->fi.panel_maker_code;
		hi->lcd_maker_code   = cd->fi.lcd_maker_code;
		hi->lcd_maker_id     = cd->pi.lcd_maker_id;
		hi->model_type       = cd->pi.model_type;
		hi->trial_type       = cd->pi.trial_type;
/* FUJITSU:2013-05-28 HMW_TOUCH_FWVER add start */
		hi->project_id       = cd->fi.project_id;
		hi->config_version   = (((cd->sysinfo.si_ptrs.cydata->cyito_verh << 8) & 0xFF00) | cd->sysinfo.si_ptrs.cydata->cyito_verl);
...
static int cyttsp4_request_direct_read_(struct cyttsp4_device *ttsp, u16 addr, void *buf, int size)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
	struct device *dev = cd->dev;
	...
	rc = cyttsp4_adap_read(cd, addr, buf, size);
	if(rc < 0){
		dev_err(dev, "%s: read failed rc=%d\n", __func__, rc);
	}

	return rc;
}

static int cyttsp4_request_direct_write_(struct cyttsp4_device *ttsp, u16 addr, const void *buf, int size)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
	struct device *dev = cd->dev;
	...
	rc = cyttsp4_adap_write(cd, addr, buf, size);
	if(rc < 0){
		dev_err(dev, "%s: write failed rc=%d\n", __func__, rc);
	}

	return rc;
}
static int cyttsp4_request_set_powermode_(struct cyttsp4_device *ttsp, enum cyttsp4_powermode_type type)
{
	...
	switch ( type ) {
	case CY_POWMODE_SUSPEND:
		dev_info(dev, "%s: SUSPEND (cd=%p)\n", __func__, cd);
		rc = cyttsp4_core_sleep(cd);
		if (rc) {
			dev_err(dev, "%s: Suspend failed rc=%d\n", __func__, rc);
		}
		break; /* case CY_POWMODE_SUSPEND : */

	case CY_POWMODE_RESUME:
		dev_info(dev, "%s: RESUME (cd=%p)\n", __func__, cd);
		rc = cyttsp4_core_wake(cd);
		if (rc) {
			dev_err(dev, "%s: Resume failed rc=%d\n", __func__, rc);
		}
		break; /* case CY_POWMODE_RESUME : */

	case CY_POWMODE_HARDRESET:
		dev_info(dev, "%s: HARD RESET (cd=%p)\n", __func__, cd);
		rc = cyttsp4_hw_hard_reset(cd);
		if (rc) {
			dev_err(dev, "%s: H/W Reset failed rc=%d\n", __func__, rc);
		}
		break; /* case CY_POWMODE_HARDRESET : */

	case CY_POWMODE_SOFTRESET:
		dev_info(dev, "%s: SOFT RESET (cd=%p)\n", __func__, cd);
		rc = cyttsp4_hw_soft_reset(cd);
		if (rc) {
			dev_err(dev, "%s: S/W Reset failed rc=%d\n", __func__, rc);
		}
		break; /* case CY_POWMODE_SOFTRESET : */

	default:
		rc = -EINVAL;
		dev_err(dev, "%s: Invalid value, type = %d, r = %d\n", __func__, type, rc);
		break;
	}
...
static int cyttsp4_request_get_information_(struct cyttsp4_device *ttsp, enum cyttsp4_information_type type, enum cyttsp4_information_option option, void *info)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
	struct device *dev = cd->dev;
	int rc = 0;
	enum cyttsp4_information_state *state = NULL;
	u8	*data = NULL;

	if (info == NULL) {
		dev_err(dev, "%s: Invalid value, type = %d, option = 0x%08x, r = %d\n", __func__, type, option, rc);
		return -EINVAL;
	}

	switch ( type ) {
	case CY_INFOTYPE_IS_PROBE_ERR:
		state = (enum cyttsp4_information_state *)info;

		*state = CY_INFOSTATE_FALSE;					/* don't care */

		/* NOTE : don't care */
		/* Touch-IC is not connected, cyttsp4_startup() is returned -ENODEV. */
		/* Other touch module(ioctl...) probe fail. */
		break; /* case CY_INFOTYPE_IS_PROBE_ERR : */
	case CY_INFOTYPE_IS_CORE_SLEEP:
		state = (enum cyttsp4_information_state *)info;

		if (cyttsp4_is_sleep( cd ) == true) {
			*state = CY_INFOSTATE_TRUE;
		} else {
			*state = CY_INFOSTATE_FALSE;
		}
		break; /* case CY_INFOTYPE_IS_CORE_SLEEP : */
	case CY_INFOTYPE_PRESS_CTRL_INFO:
		data = (u8*)info;		/* u8 dat[8] : &dat[0] */

		data[0] = (u8)cd->press_setting_val;
		data[1] = (u8)cd->press_setting_req;
		data[2] = cd->press_normalize_nv_read;
		data[3] = cd->press_normalize_sts;
		data[4] = cd->press_force_calib_sts;
		/* [5:7] reserved */
		break; /* case CY_INFOTYPE_PRESS_CTRL_INFO : */
	default:
		rc = -EINVAL;
		dev_err(dev, "%s: Invalid value, type = %d, option = 0x%08x, r = %d\n", __func__, type, option, rc);
		break; /* default : */
...
static bool cyttsp4_is_sleep(struct cyttsp4_core_data *cd)
{
	...
	_cyttsp4_dlog(cd->dev, "%s = %d(ss = %d).\n", __func__, rc, ss);

	return rc;
}

/* FUJITSU:2013-05-09 HMW_TTDA_SET_CHARGING_PARAM add start */
static bool cyttsp4_is_active(struct cyttsp4_core_data *cd)
{
	bool rc = false;
	enum cyttsp4_mode mode;
	...
	if(mode == CY_MODE_OPERATIONAL){
		rc = true;
	} else if(mode == CY_MODE_CAT){
		rc = true;
	} else if(mode == CY_MODE_STARTUP){
		rc = true;
	}

	_cyttsp4_dlog(cd->dev, "%s = %d(mode = %d).\n", __func__, rc, mode);

	return rc;
}
/* FUJITSU:2013-05-09 HMW_TTDA_SET_CHARGING_PARAM add end */

static int cyttsp4_request_set_ioctl_handle_(struct cyttsp4_device *ttsp, void *ioctl)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
...
/* FUJITSU:2013-05-17 HMW_TOUCH_RUNTIME_CONFIG mod start */
static int cyttsp4_set_Pressure_ctrl_main(struct cyttsp4_core_data *cd, bool IsForce, bool IsSet, enum cyttsp4_information_state *info)
{
	struct device *dev = cd->dev;
	enum cyttsp4_information_state	ctl_info;
	int 	rc = 0;
	u32		io_val = 0;

/* FUJITSU:2013-05-28 HMW_TOUCH_MULTI_MODEL add start */
	if (_cyttsp4_core_is_support(cd, CY_CORE_SUPPORT_PRESS) != 1) {
		if (IsSet != true) {
			*info	= CY_INFOSTATE_FALSE;
		}
		_cyttsp4_dlog(dev, "%s not support Press ctrl.\n", __func__ );
		return 0;
	}
/* FUJITSU:2013-05-28 HMW_TOUCH_MULTI_MODEL add end */

	if (IsSet == true) {
		_cyttsp4_dlog(dev, "%s set [S], req = %d, val = %d, fc = %d.\n", __func__, *info, cd->press_setting_val, IsForce);
		/*
		 * Touch IC is sleeping, it is only set as RAM.
		 * However, it writes in compulsorily at the time of Resnume.
		 * Being in the state which can use I2C.
		 */
		cd->press_setting_req = (*info == CY_INFOSTATE_TRUE ? CY_INFOSTATE_TRUE : CY_INFOSTATE_FALSE);
		if ((IsForce != true) && (cyttsp4_is_sleep(cd) == true)) {

			_cyttsp4_hlog(dev, "%s : not ready, req = %d.\n", __func__, cd->press_setting_req);
			goto exit;
		}
		if (cd->press_setting_req == cd->press_setting_val) {
			_cyttsp4_dlog(dev, "%s : already set, val = %d.\n", __func__, cd->press_setting_val);
			goto exit;
		}
		io_val		= (*info == CY_INFOSTATE_TRUE ? CY_ENABLE : CY_DISABLE);
		rc = cyttsp4_set_parameter( cd , CY_PRESS_SENSOR_ENABLE, CY_PRESS_SENSOR_ENABLE_PARM_SZ, io_val);
		if (rc < 0) {
			_cyttsp4_elog(dev, "%s : PressSensCtrl(%d) = %d.\n", __func__, io_val, rc);
			goto exit;
		}

		cd->pdata->mt_setting(cd->pdata, (*info == CY_INFOSTATE_TRUE ? 1/* set*/ : 0 /*reset */), CY_MT_FLAG_PRESS, dev);
		cd->press_setting_val = *info;

		_cyttsp4_core_runtime_debug(cd , __func__, CY_PRESS_SENSOR_ENABLE, io_val);		/* debug */

		_cyttsp4_dlog(dev, "%s set [E], val = %d.\n", __func__, cd->press_setting_val);
	} else {
		_cyttsp4_dlog(dev, "%s get [S], fc = %d.\n", __func__, IsForce);
		if ((IsForce != true) && (cyttsp4_is_sleep(cd) == true)) {
			*info = (cd->press_setting_req == CY_INFOSTATE_TRUE ? CY_INFOSTATE_TRUE : CY_INFOSTATE_FALSE);

			_cyttsp4_hlog(dev, "%s : not ready, get = %d.\n", __func__, cd->press_setting_req);
			goto exit;
		}
		rc = cyttsp4_get_parameter( cd, CY_PRESS_SENSOR_ENABLE, &io_val);
		if (rc < 0) {
			_cyttsp4_elog(dev, "%s : PressSensCtrl = %d.\n", __func__, rc);
			goto exit;
		}
		ctl_info	= (io_val == CY_ENABLE ? CY_INFOSTATE_TRUE : CY_INFOSTATE_FALSE);

		if (cd->press_setting_val == CY_INFOSTATE_INIT) {
			cd->press_setting_val = ctl_info;
		}
		if (cd->press_setting_val != ctl_info) {
			_cyttsp4_elog(dev, "%s : not equal value = (core=%d, hw=%d).\n", __func__, cd->press_setting_val, ctl_info);
		}
		*info = cd->press_setting_val;

		_cyttsp4_dlog(dev, "%s get [E], val = %d.\n", __func__, *info);
	}
...
static int cyttsp4_set_ChargerArmor_ctrl_main(struct cyttsp4_core_data *cd, bool IsForce, bool IsSet, enum cyttsp4_information_state *info)
{
	struct device *dev = cd->dev;
	enum cyttsp4_information_state	ctl_info;
	int		rc = 0;
	...
	if (IsSet == true) {
		_cyttsp4_dlog(dev, "%s set [S], req = %d, val = %d, fc = %d.\n", __func__, *info, cd->charger_armor_setting_val, IsForce);
		if (cd->nv_info->not_use_charger_armor == CY_TOUCH_DBG_ON) {		/* for debug */
			_cyttsp4_hlog(dev, "%s : charger armor skip.\n", __func__);
			goto exit;
		}
		 ...
		 * Being in the state which can use I2C.
		 */
		cd->charger_armor_setting_req = (*info == CY_INFOSTATE_TRUE ? CY_INFOSTATE_TRUE : CY_INFOSTATE_FALSE);
		if (((IsForce != true) && (cyttsp4_is_sleep(cd) == true)) || (cyttsp4_is_active(cd) == false)) {
			_cyttsp4_hlog(dev, "%s : not ready, req = %d.\n", __func__, cd->charger_armor_setting_req);
			goto exit;
		}

		if (cd->charger_armor_setting_req == cd->charger_armor_setting_val) {
			_cyttsp4_dlog(dev, "%s : already set, val = %d.\n", __func__, cd->charger_armor_setting_val);
			goto exit;
		}

		io_val		= (*info == CY_INFOSTATE_TRUE ? CY_ENABLE : CY_DISABLE);
		rc = cyttsp4_set_parameter( cd , CY_CHARGER_STATUS, CY_CHARGER_STATUS_PARM_SZ, io_val);
		if (rc < 0) {
			_cyttsp4_elog(dev, "%s : ChargerArmorCtrl(%d) = %d.\n", __func__, io_val, rc);
			goto exit;
		}

		cd->charger_armor_setting_val = *info;

		_cyttsp4_core_runtime_debug(cd , __func__, CY_CHARGER_STATUS, io_val);		/* debug */

		_cyttsp4_dlog(dev, "%s set [E], val = %d.\n", __func__, cd->charger_armor_setting_val);
	} else {
		_cyttsp4_dlog(dev, "%s get [S], fc = %d.\n", __func__, IsForce);
		if (((IsForce != true) && (cyttsp4_is_sleep(cd) == true))) {
			*info = (cd->charger_armor_setting_req == CY_INFOSTATE_TRUE ? CY_INFOSTATE_TRUE : CY_INFOSTATE_FALSE);
			_cyttsp4_hlog(dev, "%s : not ready, get = %d.\n", __func__, cd->charger_armor_setting_req);
			goto exit;
		}

		rc = cyttsp4_get_parameter( cd, CY_CHARGER_STATUS, &io_val);
		if (rc < 0) {
			_cyttsp4_elog(dev, "%s : ChargerArmorCtrl = %d.\n", __func__, rc);
			goto exit;
		}

		ctl_info	= (io_val == CY_ENABLE ? CY_INFOSTATE_TRUE : CY_INFOSTATE_FALSE);

		if (cd->charger_armor_setting_val == CY_INFOSTATE_INIT) {
			cd->charger_armor_setting_val = ctl_info;
		}
		if (cd->charger_armor_setting_val != ctl_info) {
			_cyttsp4_elog(dev, "%s : not equal value (core=%d, hw=%d).\n", __func__, cd->charger_armor_setting_val, ctl_info);
		}
		*info = cd->charger_armor_setting_val;
...
static int cyttsp4_request_set_runtime_parm_(struct cyttsp4_device *ttsp, u8 param_id, u8 param_size, u32 param_value)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
	int rc = 0;
...
static int cyttsp4_request_get_runtime_parm_(struct cyttsp4_device *ttsp, u8 param_id, u32 *param_value)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
	int rc = 0;
...
/*
 * Set RunTime Config for resume.
 */
static int cyttsp4_set_runtime_config_for_resume(struct cyttsp4_core_data *cd)
{
	enum cyttsp4_information_state	ctl_info;
	struct device *dev = cd->dev;
	int rc = 0;

	_cyttsp4_dlog(dev, "%s [S].\n", __func__);

	/* Charger Armor Ctrl */
	ctl_info = cd->charger_armor_setting_req;	/* FUJITSU:2013-05-17 HMW_TOUCH_RUNTIME_CONFIG mod */
	rc = cyttsp4_set_ChargerArmor_ctrl_main(cd, true/* force */, true/* set */, &ctl_info);
	if (rc < 0) {
		_cyttsp4_elog(dev, "%s : set Charger Armor ctrl fail = %d, val = %d.\n", __func__, rc, ctl_info);
	}

	/* Press Sensor Ctrl */
	ctl_info = cd->press_setting_req;			/* FUJITSU:2013-05-17 HMW_TOUCH_RUNTIME_CONFIG mod */
	rc = cyttsp4_set_Pressure_ctrl_main(cd, true/* force */, true/* set */, &ctl_info);
	if (rc < 0) {
		_cyttsp4_elog(dev, "%s : set Pressure ctrl fail = %d, val = %d.\n", __func__, rc, ctl_info);
	}

/* FUJITSU:2013-07-05 HMW_TOUCH_PRESS_FREQ_CHG add start */
	if (0 != cd->nv_info->press_noise_param[ CY_TOUCH_PRS_BTN_DEF_ALT_TX_PERIOD_OFST ]) {
		_cyttsp4_dlog(dev, "%s : BTN_DEFAULT_ALT_TX_PERIOD(nv) = 0x%02X.\n", __func__, cd->nv_info->press_noise_param[ CY_TOUCH_PRS_BTN_DEF_ALT_TX_PERIOD_OFST ]);
		rc = cyttsp4_set_parameter( cd , CY_BTN_DEFAULT_ALT_TX_PERIOD, CY_BTN_DEFAULT_ALT_TX_PERIOD_PARM_SZ, (u32)cd->nv_info->press_noise_param[ CY_TOUCH_PRS_BTN_DEF_ALT_TX_PERIOD_OFST ]);
		if (rc < 0) {
			_cyttsp4_elog(dev, "%s : set CY_BTN_DEFAULT_ALT_TX_PERIOD fail = %d, val = %d.\n", __func__, rc, ctl_info);
		} else {
			_cyttsp4_core_runtime_debug(cd , __func__, CY_BTN_DEFAULT_ALT_TX_PERIOD, (u32)cd->nv_info->press_noise_param[ CY_TOUCH_PRS_BTN_DEF_ALT_TX_PERIOD_OFST ]);		/* debug */
		}
	}
/* FUJITSU:2013-07-05 HMW_TOUCH_PRESS_FREQ_CHG add end */

/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_FORCE_CALIB add start */
	if (cd->nv_info->press_resume_force_calib == CY_TOUCH_DBG_ON) {	/*	132HMW_Pending!.....	*/
		cd->press_force_calib_sts = 0x00;
		rc = _cyttsp4_core_do_force_calib( cd );
		if (rc < 0) {
			_cyttsp4_elog(dev, "%s : do force calib fail = %d, val = %d.\n", __func__, rc, ctl_info);
			cd->press_force_calib_sts = 0xD1;
		} else {
			/* success */
			cd->press_force_calib_sts = 0x01;
		}
	}
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_FORCE_CALIB add end */
	_cyttsp4_dlog(dev, "%s [E].\n", __func__);

	return rc;
}
/* FUJITSU:2013-05-07 HMW_TTDA_RUNTIME_CONFIG add end */
/* FUJITSU:2013-05-09 HMW_TTDA_SET_CHARGING_PARAM add start */
static int _cyttsp4_battery_info(unsigned int batt_status, void *data)
{
	struct cyttsp4_core_data *cd = data;
	struct device *dev = cd->dev;
	enum cyttsp4_information_state	ctl_info;
	int retval = 0;

//	_cyttsp4_dlog(dev, "%s [S] (bat_sts=%x)", __func__, batt_status);
	printk("%s [S] (bat_sts=%x).\n", __func__, batt_status);
	if (cd == NULL){
		_cyttsp4_elog(dev, "%s :parameter error \n", __func__);
		goto _cyttsp4_battery_info_exit;
	}

	switch(batt_status){
	case POWER_SUPPLY_STATUS_CHARGING:
	case POWER_SUPPLY_STATUS_FULL:
		ctl_info = CY_INFOSTATE_TRUE;
		break;
	default :
		ctl_info = CY_INFOSTATE_FALSE;
		break;
	}

	retval = cyttsp4_set_ChargerArmor_ctrl_main(cd, false, true, &ctl_info);

_cyttsp4_battery_info_exit:
//	_cyttsp4_dbg(ts, CY_DBG_LVL_0,
//		"[TPD] %s: (%d,%d,0x%X), rs(%d,%d,%d,%d,%d,%d,%d,%d), hm(%d,%d,%d), lo(%d)\n",
//		__func__, ts->touch_count_up, ts->touch_count_down, ts->firmware_version,
//		ts->rep_stat_num[ 0 ], ts->rep_stat_num[ 1 ], ts->rep_stat_num[ 2 ], ts->rep_stat_num[ 3 ],
//		ts->rep_stat_num[ 4 ], ts->rep_stat_num[ 5 ], ts->rep_stat_num[ 6 ], ts->rep_stat_num[ 7 ],
//		ts->hst_mode_num[ 0 ], ts->hst_mode_num[ 1 ], ts->hst_mode_num[ 2 ], ts->tt_stat_num[ 1 ]);

//	_cyttsp4_dlog(dev, "%s [E]", __func__);
	printk("%s [E].\n", __func__);
	return retval;
}
/* FUJITSU:2013-05-09 HMW_TTDA_SET_CHARGING_PARAM add start */
...
static int cyttsp4_request_set_pressure_ctrl_(struct cyttsp4_device *ttsp, bool IsSet, enum cyttsp4_information_state *info)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
	int rc = 0;

	rc = cyttsp4_set_Pressure_ctrl_main(cd, false/* force */, IsSet/* set or get */, info);

	return rc;
}
/* FUJITSU:2013-04-23 HMW_TTDA_PRESS add end */
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add start */
static int cyttsp4_request_set_normalize_mode_(struct cyttsp4_device *ttsp, bool IsSet, enum cyttsp4_information_state *info)
{
	int rc = -1;
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
	u8 normalize_mode = CY_DISABLE;

	if (IsSet == true){
		if (*info == CY_INFOSTATE_TRUE) {
			normalize_mode = CY_ENABLE;
		} else {
			normalize_mode = CY_DISABLE;
		}
		rc = _cyttsp4_core_set_press_normalize_mode(cd, normalize_mode);
	} else {
		rc = _cyttsp4_core_get_press_normalize_mode(cd, &normalize_mode);
		if (normalize_mode == CY_ENABLE) {
...
exit:
/* FUJITSU:2013-06-18 HMW_TOUCH_WATCHDOG_TEST mod start */
	if (restart){
		cyttsp4_queue_startup(cd);
		cyttsp4_set_led(cd, SET_RECOVERY_LED_ON);
	} else {
		cyttsp4_start_wd_timer(cd);
}
/* FUJITSU:2013-06-18 HMW_TOUCH_WATCHDOG_TEST mod end */
}
#else
static void cyttsp4_watchdog_work(struct work_struct *work)
{
...
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add start */
#if 0
__sub_start____set_press_normalization____(){}
#endif

/*
 * wait complete
 */
static int _cyttsp4_core_complete_wait( struct cyttsp4_core_data *cd , int wait_ms)
{
	...
	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s: [S], ms = %d\n", __func__, wait_ms);

	if (wait_ms == 0) {
		/* false safe ... */
		wait_ms = CY_WAITCMD_SLEEP_PRESS;
	}

	for (count = 0; count < limit; count++) {
		_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: try = %d.\n", __func__, count);
		ret = _cyttsp4_core_i2c_read_________(cd, CY_COMMAND_REG, 1, &read_cmd_reg);
		if (ret < 0) {
			dev_err( _COR_REF_DEV(cd) , "%s: Command I2C error! \n", __func__);
			break;
		} else if (count > limit) {
			dev_err( _COR_REF_DEV(cd) , "%s: Command timeout! \n", __func__);
			ret = -EIO;
			break;
		}
	...
	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s: [E], ret = %d, status = 0x%02X, c = %d\n", __func__, ret, read_cmd_reg , count);

	return ret;
}


static int _cyttsp4_core_set_normalization_1parm(struct cyttsp4_core_data *cd, u8 *nv_data, int size, int point_num)
{
	int		retval = 0;
	unsigned char cmd_data[ IC_PRS_CMD_DATA_SIZE ];

	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s [S] (point = %d).\n", __func__, point_num);

	/* W 24 03 [D0] [D1] [D2] [D3] [D4] [D5]; set parameter to Normalization Parm(Point xx) */
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: set parameter to Normalization Parm.\n", __func__);

	memset((void*)&cmd_data[0], 0x00, sizeof (cmd_data));
	memcpy(&cmd_data[0], &nv_data[0], size);

	retval = _cyttsp4_core_i2c_write_________(cd, 0x03, size, &cmd_data[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed Write set parameter to Normalization Parm (ret=%d)\n", __func__, retval);
		goto cyttsp4_core_set_normalization_1parm;
	}

	/* W 24 02 14; <Set Normalization Parameter> exec */
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: <Set Normalization Parameter> exec.\n", __func__);
	memset((void*)&cmd_data[0], 0x00, sizeof (cmd_data));
	cmd_data[0]			= CY_SET_NORMALIZATION_PARM;		/* 0x14 */

	retval = _cyttsp4_core_i2c_write_________(cd, CY_COMMAND_REG, 1, &cmd_data[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed Write <Set Normalization Parameter> exec (ret=%d)\n", __func__, retval);
		goto cyttsp4_core_set_normalization_1parm;
	}

	retval = 0;

cyttsp4_core_set_normalization_1parm:

	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s [E] = %d, (point = %d).\n", __func__, retval, point_num);
...
/*
 * set normalization parm (9 points)
 *
 * LIMIT : Be sure to use it in CAT mode.
 */
static int _cyttsp4_core_press_normalize_parm(struct cyttsp4_core_data *cd)
{
	u8		nv_data[ CY_IC_NORMALIZE_PARM_SIZE + 1 ];
	int		retval = 0;
	int		ii;

	memset((void*)&nv_data[0], 0x00, sizeof (nv_data));
	retval = get_nonvolatile(&nv_data[0], 47153, CY_IC_NORMALIZE_PARM_SIZE);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed Read NV(Nomalize Parm)\n", __func__);
		memcpy(&nv_data[0], &cDefault_NormalizeParm[0], CY_IC_NORMALIZE_PARM_SIZE);
		cd->press_normalize_nv_read = 0xF1;
	} else {
		cd->press_normalize_nv_read = 0x01;
	}
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add start */
	if (cd->nv_info->data_io_debug == CY_TOUCH_DBG_ON) {
		_cyttsp4_buf_dump(&nv_data[0], CY_IC_NORMALIZE_PARM_SIZE, "Normalize Parm", CY_LOGLV0);
	}
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add end */

	for (ii = 0; ii < CY_IC_NORMALIZE_POINT_NUM; ii++) {
		retval = _cyttsp4_core_set_normalization_1parm(cd, &nv_data[ ii * CY_IC_NORMALIZE_1BLOCK_SIZE ], CY_IC_NORMALIZE_1PARM_SIZE, ii);
		if (retval < 0) {
			dev_err( _COR_REF_DEV(cd) , "%s: set normalize parm error = %d(point = %d).\n", __func__, retval, ii);
			goto error;
		}
	}
...
/*
 * set normalization run time config
 *
 * LIMIT : Be sure to use it in Ope mode.
 */
static int _cyttsp4_core_press_normalize_runtime_config(struct cyttsp4_core_data *cd)
{
	int		retval = 0;
	u32		set_val = 0;
	u8		parm_id = 0;
	...
	/*  W 24 03 5D 02 03 E8;	Set Active Calibration Time(temp_val ms) */
	set_val		= 1000;		/* xxxx ms */
	parm_id		= CY_ACTIVE_CALIB_TIM;
	parm_sz		= CY_ACTIVE_CALIB_TIM_PARM_SZ;
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: Set Active Calibration Time(%02X, %d : %d ms).\n", __func__, parm_id, parm_sz, set_val);

	retval = cyttsp4_set_parameter(cd , parm_id, parm_sz, set_val);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed RT Parm <Active Calibration Time> (ret=%d)\n", __func__, retval);
		goto cyttsp4_core_set_normalization_runtime_parm;
	}
	_cyttsp4_core_runtime_debug(cd , __func__,  parm_id, set_val);			/* debug */

	/*  W 24 03 5E 02 0F A0; Set Periodical Calibration Time(set_val ms) */
	set_val		= 4000;		/* xxxx ms */
	parm_id		= CY_PERIODICAL_CALIB_TIM;
	parm_sz		= CY_PERIODICAL_CALIB_TIM_PARM_SZ;
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: Set Periodical Calibration Time(%02X, %d : %d ms).\n", __func__, parm_id, parm_sz, set_val);

	retval = cyttsp4_set_parameter(cd , parm_id, parm_sz, set_val);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed RT Parm <Periodical Calibration Time> (ret=%d)\n", __func__, retval);
		goto cyttsp4_core_set_normalization_runtime_parm;
	}
	_cyttsp4_core_runtime_debug(cd , __func__, parm_id, set_val);			/* debug */

	/*  W 24 03 60 01 03; Set Press Threshold(set_val Count) */
	set_val		= CY_PRESS_TH_VAL_NORMAL;		/* xxxx ms */
	parm_id		= CY_PRESS_THRESHOLD;
	parm_sz		= CY_PRESS_THRESHOLD_PARM_SZ;
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: Set Press Threshold(%02X, %d : %d count).\n", __func__, parm_id, parm_sz, set_val);

	retval = cyttsp4_set_parameter(cd , parm_id, parm_sz, set_val);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed RT Parm <Press Threshold> (ret=%d)\n", __func__, retval);
		goto cyttsp4_core_set_normalization_runtime_parm;
	}
	_cyttsp4_core_runtime_debug(cd , __func__, parm_id, set_val);			/* debug */

	/*  W 24 03 61 01 01;	Set Press Sensor Enable(Enable(01h)) */
	set_val 	= CY_ENABLE;	/* enable */
	parm_id		= CY_PRESS_SENSOR_ENABLE;
	parm_sz		= CY_PRESS_SENSOR_ENABLE_PARM_SZ;
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: Set Press Sensor Enable(%02X, %d : 0x%02X).\n", __func__, parm_id, parm_sz, set_val);

	retval = cyttsp4_set_parameter(cd , parm_id, parm_sz, set_val);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed RT Parm <Press Sens Enable> (ret=%d)\n", __func__, retval);
		goto cyttsp4_core_set_normalization_runtime_parm;
	}
	_cyttsp4_core_runtime_debug(cd , __func__, parm_id, set_val);			/* debug */
...
cyttsp4_core_set_normalization_runtime_parm:
...
static int _cyttsp4_core_set_press_normalize_mode(struct cyttsp4_core_data *cd, u8 normalize_mode)
{
	...
	/* 00:Disable, 01Enable */
	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s [S] (mode = 0x%02X).\n", __func__, normalize_mode);

	/* W 24 03 01 or W 24 03 00;	set parameter to Normalization Mode */
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: set parameter to Normalization Mode.\n", __func__);

	memset((void*)&cmd_data[0], 0x00, sizeof (cmd_data));
	cmd_data[0]		= normalize_mode;

	retval = _cyttsp4_core_i2c_write_________(cd, 0x03, 1, &cmd_data[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed Write set parameter to Normalization Mode (ret=%d)\n", __func__, retval);
		goto cyttsp4_core_set_press_normalize_mode;
	}

	/* W 24 02 12	set Normalization Mode Cmd Exec	*/
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: <set parameter to Normalization Mode> exec.\n", __func__);

	memset((void*)&cmd_data[0], 0x00, sizeof (cmd_data));
	cmd_data[0]			= CY_SET_NORMALIZATION_MODE;		/* 0x12 */

	retval = _cyttsp4_core_i2c_write_________(cd, CY_COMMAND_REG, 1, &cmd_data[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed Write <set parameter to Normalization Mode> exec (ret=%d)\n", __func__, retval);
		goto cyttsp4_core_set_press_normalize_mode;
	}

	/* (wait for communications interrupt) */
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: wait <set parameter to Normalization Mode> exec comp.\n", __func__);
	retval = _cyttsp4_core_complete_wait(cd, CY_WAITCMD_SLEEP_PRESS);
	if (retval < 0) {
		goto cyttsp4_core_set_press_normalize_mode;
	}

	/* Check Result Data */
	/* W 24 03; R 24 xx ; set Normalization Mode comp status conf.*/
	memset(raw_return, 0x00, sizeof(raw_return));

	retval = _cyttsp4_core_i2c_read_________(cd, 0x03, 1, &raw_return[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed Read set Normalization Mode comp status (ret=%d)\n", __func__, retval);
		goto cyttsp4_core_set_press_normalize_mode;
	}

	if (raw_return[0] != 0x00) {		/* 00 : Success, 01 : Fail */
		dev_err( _COR_REF_DEV(cd) , "%s: set Normalization Mode comp status Failure (status=0x%02X)\n", __func__, raw_return[0]);
		retval = -EINVAL;
		goto cyttsp4_core_set_press_normalize_mode;
	}
...
cyttsp4_core_set_press_normalize_mode:

	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s [E] = %d, (set_mode = 0x%02X).\n", __func__, retval, normalize_mode);
	return retval;
}
...
static int _cyttsp4_core_get_press_normalize_mode(struct cyttsp4_core_data *cd, u8 *normalize_mode)
{
	...
	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s [S].\n", __func__ );

	/* W 24 02 13;	get Normalization Mode Cmd Exec */
	memset(raw_return, 0x00, sizeof(raw_return));
	cmd_data[0]			= CY_GET_NORMALIZATION_MODE;		/* 0x13 */
	retval = _cyttsp4_core_i2c_write_________(cd, CY_COMMAND_REG, 1, &cmd_data[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed Write <get parameter to Normalization Mode> exec (ret=%d)\n", __func__, retval);
		goto cyttsp4_core_get_press_normalize_mode;
	}
	/* (wait for communications interrupt) */
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: wait <get parameter to Normalization Mode> exec comp.\n", __func__);
	retval = _cyttsp4_core_complete_wait(cd, CY_WAITCMD_SLEEP_PRESS);
	if (retval < 0) {
		goto cyttsp4_core_get_press_normalize_mode;
	}
	/* W 24 03; R 24 xx ; get Normalization Mode.*/
	memset(raw_return, 0x00, sizeof(raw_return));
	retval = _cyttsp4_core_i2c_read_________(cd, 0x03, 1, &raw_return[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , " %s: Failed Read set Normalization Mode comp status (ret=%d)\n", __func__, retval);
	...
	*normalize_mode = raw_return[0];
	/* success */
	retval = 0;

cyttsp4_core_get_press_normalize_mode:

	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s [E] = %d, (get_mode = 0x%02X).\n", __func__, retval, *normalize_mode);
	return retval;
}
 ...
 * set normalization
 *
 * LIMIT : Be sure to use it in CAT mode and EXCLUSIVE.
 */
static int _cyttsp4_core_press_normalize_mode(struct cyttsp4_core_data *cd, u8 set_val)
{
	int		ii;
	u8	get_val = 0x00;
	int	retval = 0;

	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s [S] val = %d.\n", __func__, set_val);

	for (ii = 0; ii < CY_ENABLE_NORMALIZE_RETRY; ii++) {
		_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s try = %d.\n", __func__, ii);

		retval = _cyttsp4_core_set_press_normalize_mode( cd, set_val );
		if (retval < 0) {
			dev_err( _COR_REF_DEV(cd) , "%s: Failed to set_press_normalize_mode.\n", __func__);
			goto exit;
		}

		retval = _cyttsp4_core_get_press_normalize_mode( cd, &get_val);
		if (retval < 0) {
			dev_err( _COR_REF_DEV(cd) , "%s: Failed to get_press_normalize_mode.\n", __func__);
			goto exit;
		}
		_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s (get = %d, expect = %d).\n", __func__, get_val, set_val);
		if (get_val == set_val) {
			break;
		}
	}

	if ( ii == CY_ENABLE_NORMALIZE_RETRY ) {
		retval = -1;
		dev_err( _COR_REF_DEV(cd) , "%s: retry over! .\n", __func__);
		goto exit;
	}

exit:
	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s [E] = %d.\n", __func__, retval);
...
static int _cyttsp4_core_press_normalize(struct cyttsp4_core_data *cd)
{
	int	retval = 0;

	/* [Section] Start of module. */
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s [S].\n", __func__);

	/* ; Change Mode to Test and Config */
	retval = set_mode(cd, CY_MODE_CAT);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Failed to switch to config mode, rc = %d, \n", __func__, retval);
		goto exit;
	}

	/* [Section] Main Routine. */
	/* Set Normalization parm (9 point) */
	retval = _cyttsp4_core_press_normalize_parm( cd );
	if (retval < 0) {
		cd->press_normalize_sts = 0xE1;
		dev_err( _COR_REF_DEV(cd) , "%s: Failed to press_normalize_parm.\n", __func__);
		goto exit;
	}

	retval = _cyttsp4_core_press_normalize_mode( cd , CY_ENABLE );
	if (retval < 0) {
		cd->press_normalize_sts = 0xE2;
		dev_err( _COR_REF_DEV(cd) , "%s: Failed to press_normalize_mode.\n", __func__);
		goto exit;
	}
...
exit:
	...
	if (retval < 0) {
		...
		retval = set_mode(cd, CY_MODE_OPERATIONAL);
	}

	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s [E] = %d.\n", __func__, retval);

	return retval;
}

/*
 * set normalization for Start up
 *
 * LIMIT : Be sure to use it in EXCLUSIVE.
 */
static int _cyttsp4_core_startup_press_normalize(struct cyttsp4_core_data *cd)
{
	int retval = 0;

	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s [S].\n", __func__);

	cd->press_normalize_nv_read = 0x00;
	cd->press_normalize_sts = 0x00;
	cd->press_force_calib_sts = 0x00;

	if (cd->nv_info->press_normalize == CY_TOUCH_DBG_ON) {	/*	132HMW_Pending!.....	*/
		retval = _cyttsp4_core_press_normalize( cd );
		if (retval < 0) {
			_cyttsp4_dbg(cd, CY_LOGLV0, "[TPD] %s error, set press normalize rc = %d.\n", __func__, retval);
			retval = 0;									/*	132HMW_Pending!.....	*/
		}

		retval = _cyttsp4_core_press_normalize_runtime_config( cd );
		if (retval < 0) {
			_cyttsp4_dbg(cd, CY_LOGLV0, "[TPD] %s error, set press normalize runtime parm rc = %d.\n", __func__, retval);
			retval = 0;									/*	132HMW_Pending!.....	*/
		}
		if (cd->press_normalize_sts == 0x00) {
			cd->press_normalize_sts = 0x01;
		}
	} else {
		_cyttsp4_dbg(cd, CY_LOGLV0, "[TPD] %s not set press normalization.\n", __func__);
	}
...
static int _cyttsp4_core_runtime_debug( struct cyttsp4_core_data *cd, const char *func, u8 reg, u32 expect_val)
{
	...
	if (cy_nv_info.data_io_debug == CY_TOUCH_DBG_ON) {
		if (cyttsp4_get_parameter(cd, reg, &get_val) == 0) {
			if (get_val != (u32)expect_val) {
				_cyttsp4_elog(cd->dev, "(Debug) %s: not equal value (0x%02X : get = %d, expect = %d)\n", func, reg, get_val, expect_val);
				rc = -1;
			}
		} else {
			_cyttsp4_elog(cd->dev, "(Debug) %s: get_runtime (0x%02X) error.\n", func, reg);	\
			rc = -2;
		}
	}
	return rc;
...
#if 0
__sub_end______set_press_normalization____(){}
#endif

/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add end */
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_FORCE_CALIB add start */
#if 0
__sub_start____press_force_calib____(){}
#endif

/*
 * set normalization
 *
 * LIMIT : Be sure to use it in OPE mode and EXCLUSIVE.
 */
static int _cyttsp4_core_do_force_calib(struct cyttsp4_core_data *cd)
{
	int	retval = 0;
	...
	_cyttsp4_dbg(cd, CY_LOGLV1, "[TPD] %s: Set ParameterID = Force calib\n", __func__);
	/* Set ParameterID = Force calibration 				W 24 03 5F 01 01 */
	/* (5F:ForceCalibration configurable parameter, 01:Parameter Size, 01:Parameter Value(initiate the force Calibration) */
	memset(cmd_data, 0x00, sizeof(cmd_data));
	cmd_data[0]			= CY_FORCE_CALIB_GIDAC;					/* 0x5F : ForceCalibration configurable parameter */
	cmd_data[1]			= CY_FORCE_CALIB_GIDAC_PARM_SZ;			/* 0x01 : Parameter Size */
	cmd_data[2]			= 0x01;									/* Parameter Value(initiate the force Calibration) */

	retval = _cyttsp4_core_i2c_write_________(cd, 0x03, 3, &cmd_data[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Set ParameterID = Force calib (ret=%d)\n", __func__, retval);
		goto _cyttsp4_core_do_force_calib;
	}

	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: Do SetParameter ID (= Force calib)\n", __func__);
	/* Do SetParameter ID (= Force calibration) 			W 24 02 03	*/
	memset(cmd_data, 0x00, sizeof(cmd_data));
	cmd_data[0]		= 0x03;
	retval = _cyttsp4_core_i2c_write_________(cd, CY_COMMAND_REG, 1, &cmd_data[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Do SetParameter ID (= Force calib) (ret=%d)\n", __func__, retval);
		goto _cyttsp4_core_do_force_calib;
	}

	/* (wait for communications interrupt)			ForceCalibration Comp Wait */
	retval = _cyttsp4_core_complete_wait(cd, CY_WAITCMD_SLEEP_PRESS);
	if (retval < 0) {
		goto _cyttsp4_core_do_force_calib;
	}

	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: ParameterID Set Force calib\n", __func__);
	/* ParameterID Set Force calibration			W 24 03 5F */
	memset((void*)&cmd_data[0], 0x00, sizeof (cmd_data));
	cmd_data[0]			= CY_FORCE_CALIB_GIDAC;					/* 0x5F : ForceCalibration configurable parameter */
	retval = _cyttsp4_core_i2c_write_________(cd, 0x03, 1, &cmd_data[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: ParameterID Set Force calib (ret=%d)\n", __func__, retval);
		goto _cyttsp4_core_do_force_calib;
	}
	
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: Do GetParameter\n", __func__);
	/* Do GetParameter					W 24 02 02 */
	memset((void*)&cmd_data[0], 0x00, sizeof (cmd_data));
	cmd_data[0]			= 0x02;

	retval = _cyttsp4_core_i2c_write_________(cd, CY_COMMAND_REG, 1, &cmd_data[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Do GetParameter (ret=%d)\n", __func__, retval);
		goto _cyttsp4_core_do_force_calib;
	}
	...
	_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: Check Result Data\n", __func__);
	/* Check Result Data :			R 24 03 xx */
	memset((void*)&raw_return[0], 0x00, sizeof (raw_return));
	retval = _cyttsp4_core_i2c_read_________(cd, 0x03, 3, &raw_return[0]);
	if (retval < 0) {
		dev_err( _COR_REF_DEV(cd) , "%s: Check Result Data (ret=%d)\n", __func__, retval);
		goto _cyttsp4_core_do_force_calib;
	}

	if (raw_return[ 2 ] == 0x00) {
		_cyttsp4_dbg(cd, CY_LOGLV2, "[TPD] %s: Do calib pres GIDAC Success\n", __func__);
	} else {
		/* 0:Success, 1:Filure */
		dev_err( _COR_REF_DEV(cd) , "%s: Calib Check Result Data (ret=%d)\n", __func__, raw_return[ 2 ]);
		retval = -1;
	}

_cyttsp4_core_do_force_calib:

	return retval;
}

static int cyttsp4_request_do_force_calib_(struct cyttsp4_device *ttsp, enum cyttsp4_information_option option)
{
	int	retval = 0;
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);

	retval = _cyttsp4_core_do_force_calib( cd );

	return retval;
}

#if 0
__sub_end______press_force_calib____(){}
#endif

/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_FORCE_CALIB add end */
/* FUJITSU:2013-05-28 HMW_TOUCH_MULTI_MODEL add start */
static int cyttsp4_request_is_support_(struct cyttsp4_device *ttsp, int check_type)
{
	/* int check_type : CY_CORE_SUPPORT_PRESS, CY_CORE_SUPPORT_HOVER... cyttsp4_core.h */
	int ret = 0;		/* 0 : not support, 1 : support */
	struct cyttsp4_core *core = ttsp->core;
	...
	ret = _cyttsp4_core_is_support(cd , check_type);

	_cyttsp4_dlog(cd->dev, "%s(type = %d) = %d.\n", __func__, check_type, ret);
...
/* FUJITSU:2013-05-07 HMW_TTDA_RUNTIME_CONFIG add start */
	cyttsp4_set_runtime_config_for_resume(cd);
/* FUJITSU:2013-05-07 HMW_TTDA_RUNTIME_CONFIG add end */
	cyttsp4_start_wd_timer(cd);
...
static int cyttsp4_startup_(struct cyttsp4_core_data *cd)
{
	...
	_cyttsp4_core_show_sw_status( cd );		/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add */
...
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add start */
	rc = _cyttsp4_core_startup_press_normalize( cd );
	if (rc < 0) {
		dev_err(cd->dev, "%s: failed to set press normalization rc=%d\n",
			__func__, rc);
		if (retry--)
			goto reset;
		goto exit;
...
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add end */
/* FUJITSU:2013-05-28 HMW_TOUCH_RUNTIME_CONFIG add start */
	cd->press_setting_val			= CY_INFOSTATE_INIT;	/* init(!= TURE/FALSE) */
	cd->charger_armor_setting_val	= CY_INFOSTATE_INIT;	/* init(!= TURE/FALSE) */
	cyttsp4_set_runtime_config_for_resume( cd );
/* FUJITSU:2013-05-28 HMW_TOUCH_RUNTIME_CONFIG add end */
...
/* FUJITSU:2013-05-17 HMW_TOUCH_RUNTIME_CONFIG mod start */
	/* FUJITSU:2013-05-28 HMW_TOUCH_MULTI_MODEL mod start */
	cd->press_setting_req			= CY_INFOSTATE_TRUE;
	cd->press_setting_val			= CY_INFOSTATE_INIT;	/* init(!= TURE/FALSE) */
	if (_cyttsp4_core_is_support(cd, CY_CORE_SUPPORT_PRESS) == 1) {
		/* support */
		cd->press_setting_req			= CY_INFOSTATE_TRUE;	/* (press on) depend on touch fw */
	} else {
		/* not support */
		cd->press_setting_req			= CY_INFOSTATE_FALSE;
		cd->press_setting_val			= CY_INFOSTATE_FALSE;
	}
	/* FUJITSU:2013-05-28 HMW_TOUCH_MULTI_MODEL mod end */
	cd->charger_armor_setting_req	= CY_INFOSTATE_FALSE;	/* cahrger off */
	cd->charger_armor_setting_val	= CY_INFOSTATE_INIT;	/* init(!= TURE/FALSE) */
/* FUJITSU:2013-05-17 HMW_TOUCH_RUNTIME_CONFIG mod start */
	/* FUJITSU:2013-05-01 HMW_TTDA_MODIFY_LOG add start */
	cd->nv_info			= cyttsp4_request_ref_nonvolatile_info(NULL);
	/* FUJITSU:2013-05-01 HMW_TTDA_MODIFY_LOG add end */

/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add start */
	memset(&cd->fi, 0xFF, sizeof(struct cyttsp4_touch_firmware_info));
	memset(&cd->pi, 0xFF, sizeof(struct cyttsp4_touch_platform_info));
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add end */
...
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER mod start */
	if(cd->pdata->pdata_init){
		cd->pdata->pdata_init(cd->pdata, &cd->fi, CY_INIT_FLAG_SET_CONFIG_PARAMS | CY_INIT_FLAG_SET_PDATA_FLAGS);
	}
	if(cd->pdata->get_platform_info){
		cd->pdata->get_platform_info(cd->pdata, &cd->pi);
	}
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER mod end */
/* FUJITSU:2013-05-09 HMW_TTDA_SET_CHARGING_PARAM add start */
	/* set battery_info */
	cd->battery_callback.callback = _cyttsp4_battery_info;
	cd->battery_callback.data = cd;
	set_msm_battery_callback_info(&cd->battery_callback);
/* FUJITSU:2013-05-09 HMW_TTDA_SET_CHARGING_PARAM add end */
...
/* FUJITSU:2013-05-09 HMW_TTDA_SET_CHARGING_PARAM mod start */
//		goto error_startup;
		goto error_sysfs_if;
/* FUJITSU:2013-05-09 HMW_TTDA_SET_CHARGING_PARAM mod end */
	}
...
/* FUJITSU:2013-05-09 HMW_TTDA_SET_CHARGING_PARAM add start */
error_sysfs_if:
	unset_set_msm_battery_callback_info();
/* FUJITSU:2013-05-09 HMW_TTDA_SET_CHARGING_PARAM add end */
error_startup:
...
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add start */
	.request_hardware_info = cyttsp4_request_hardware_info_,
/* FUJITSU:2013-04-02 HMW_MULTI_VENDER add end */
/* FUJITSU:2013-05-17 HMW_TOUCH_IOCTL add start */
	.request_direct_write = cyttsp4_request_direct_write_,
	.request_direct_read = cyttsp4_request_direct_read_,
	.request_set_powermode = cyttsp4_request_set_powermode_,
	.request_get_information = cyttsp4_request_get_information_,
/* FUJITSU:2013-05-17 HMW_TOUCH_IOCTL add end */
/* FUJITSU:2013-04-23 HMW_TTDA_PRESS add start */
	.request_set_pressure_ctrl = cyttsp4_request_set_pressure_ctrl_,
/* FUJITSU:2013-04-23 HMW_TTDA_PRESS add end */
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add start */
	.request_set_normalize_mode = cyttsp4_request_set_normalize_mode_,
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_NORMALIZE_CORE add end */
/* FUJITSU:2013-05-07 HMW_TTDA_RUNTIME_CONFIG add start */
	.request_set_ioctl_handle = cyttsp4_request_set_ioctl_handle_,
/* FUJITSU:2013-05-07 HMW_TTDA_RUNTIME_CONFIG add end */
/* FUJITSU:2013-05-17 HMW_TOUCH_RUNTIME_CONFIG add start */
	.request_set_runtime_parm = cyttsp4_request_set_runtime_parm_,
	.request_get_runtime_parm = cyttsp4_request_get_runtime_parm_,
/* FUJITSU:2013-05-17 HMW_TOUCH_RUNTIME_CONFIG add end */
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_FORCE_CALIB add start */
	.request_do_force_calib = cyttsp4_request_do_force_calib_,
/* FUJITSU:2013-05-17 HMW_TOUCH_PRESS_FORCE_CALIB add end */
/* FUJITSU:2013-05-28 HMW_TOUCH_MULTI_MODEL add start */
	.request_is_support = cyttsp4_request_is_support_,
/* FUJITSU:2013-05-28 HMW_TOUCH_MULTI_MODEL add end */
...
/* FUJITSU:2013-05-01 HMW_TTDA_MODIFY_LOG add start */
	_cyttsp4_get_all_nor_info( &cy_nv_info );
/* FUJITSU:2013-05-01 HMW_TTDA_MODIFY_LOG add end */
