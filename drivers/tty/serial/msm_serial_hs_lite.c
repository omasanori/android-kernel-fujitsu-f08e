...
/* FUJITSU:2013-01-25 UART GPIO Setting start */
#include <linux/gpio.h>
#include <linux/nonvolatile_common.h>

#define APNV_UART_CONSOLE_ENABLE	0xA062
/* FUJITSU:2013-01-25 UART GPIO Setting end */
...
/* FUJITSU:2013-01-10 NFC Driver add start */
#ifdef CONFIG_NFC_FJ
#include "../../nfc/snfc_ioctl.h"
#endif
/* FUJITSU:2013-01-10 NFC Driver add end   */
...
/* FUJITSU:2013-01-15 Felica Driver add start */
	...
	if ((msm_hsl_port->uart.line) == FELICA_UART) {
		if(makercmd_mode != WKUP_MODE_MC) {
			uid_chk = sys_getuid();
		
			if(uid_chk != FCCTL_UID2) {
/* FUJITSU:2013-01-10 NFC Driver add start */
#ifdef NFC_UID_CHECK
				if( uid_chk != NFCCTL_UID7 ) {
#endif
/* FUJITSU:2013-01-10 NFC Driver add end   */
				printk("Felica_UART :open() NG UID = 0x%lx\n",uid_chk);
				return -EIO;
/* FUJITSU:2013-01-10 NFC Driver add start */
#ifdef NFC_UID_CHECK
				}
...
/* FUJITSU:2013-01-10 uart gsbi add start */
	{
		.uart = {
			.iotype = UPIO_MEM,
			.ops = &msm_hsl_uart_pops,
...
/* FUJITSU:2013-01-10 uart gsbi add end */
};
...
/* FUJITSU:2013-01-25 UART GPIO Setting start */
	/* UART2 */
	if( pdev->id == 2 ){
		if (get_nonvolatile((void*)&uart_logflg, APNV_UART_CONSOLE_ENABLE, sizeof(uart_logflg)) > 0) {
			if( !uart_logflg ) {
				printk(KERN_INFO "msm_serial_hsl: uart_log no put\n");
				gpio_tlmm_config(GPIO_CFG(23, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN,GPIO_CFG_2MA),GPIO_CFG_DISABLE);
				gpio_tlmm_config(GPIO_CFG(22, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,GPIO_CFG_2MA),GPIO_CFG_DISABLE);
			}
			else {
				printk(KERN_INFO "msm_serial_hsl: uart_logflg put\n");
				gpio_tlmm_config(GPIO_CFG(23, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN,GPIO_CFG_2MA),GPIO_CFG_DISABLE);
				gpio_tlmm_config(GPIO_CFG(22, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,GPIO_CFG_2MA),GPIO_CFG_DISABLE);
			}
		}
		else {
			pr_err(KERN_ERR "get_nonvolatile failed\n");
			gpio_tlmm_config(GPIO_CFG(23, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN,GPIO_CFG_2MA),GPIO_CFG_DISABLE);
			gpio_tlmm_config(GPIO_CFG(22, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,GPIO_CFG_2MA),GPIO_CFG_DISABLE);
		}
	}
