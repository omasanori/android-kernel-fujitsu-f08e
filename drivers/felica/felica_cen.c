/*
 * linux/drivers/felica/felica_cen.c
 *
...
#define DEV_NAME "felica_cen"
#define CLASS_NAME "felica_cen"
...
//// SW //////////////////////////////////////////////////////
static int felica_cen_sw_init(void)
{
#ifdef FELICA_DEBUG_SUPPORT
	printk(KERN_INFO DEV_NAME ":sw_init() [IN]\n");
#endif
	return felica_cen_hw_init();
}

static int felica_cen_sw_exit(void)
{
#ifdef FELICA_DEBUG_SUPPORT
	printk(KERN_INFO DEV_NAME ":sw_exit() [IN]\n");
#endif
	return felica_cen_hw_exit();
}

static int felica_cen_sw_open(void)
{
#ifdef FELICA_DEBUG_SUPPORT
...
static int felica_cen_sw_close(void)
{
#ifdef FELICA_DEBUG_SUPPORT
...
static int felica_cen_sw_write(char val)
{
#ifdef FELICA_DEBUG_SUPPORT
	...
	if (val == 0) {
		...
		return felica_cen_hw_write(FELICA_LOCK);
	} else if (val == 1) {
		printk(KERN_INFO DEV_NAME":write() set 1\n");
		return felica_cen_hw_write(FELICA_UNLOCK);
	} else {
...
static int felica_cen_sw_read(void)
{
#ifdef FELICA_DEBUG_SUPPORT
	printk(KERN_INFO DEV_NAME ":sw_read() [IN]\n");
#endif
	return felica_cen_hw_read();
}

//// IF //////////////////////////////////////////////////////
static int felica_cen_if_init(void)
{
#ifdef FELICA_DEBUG_SUPPORT
	printk(KERN_INFO DEV_NAME ":if_init() [IN]\n");
#endif
	return felica_cen_sw_init();
}

static int felica_cen_if_exit(void)
{
#ifdef FELICA_DEBUG_SUPPORT
	printk(KERN_INFO DEV_NAME ":if_exit() [IN]\n");
#endif
	return felica_cen_sw_exit();
}

static int felica_cen_if_open(struct inode *inode, struct file *filp)
{
	int ret = 0;
...
/* FUJITSU:2013-04-11 mod_start */
			if(((system_rev & 0xF0) == 0x50) || ((system_rev & 0xF0) == 0x40) ||
			   ((system_rev & 0xF0) == 0x10)) {
/* FUJITSU:2013-04-11 mod_end */
				if (unlikely((uid_chk != FCCTL_UID1) && (uid_chk != FCCTL_UID2) &&
				             (uid_chk != FCCTL_UID3) && (uid_chk != FCCTL_UID4))) {
					printk(KERN_ERR DEV_NAME ":open() NG UID = 0x%lx\n", uid_chk);
					return -EIO;
				}
			} else if((system_rev & 0xF0) == 0x60) {
				if (unlikely((uid_chk != FCCTL_UID1) && (uid_chk != FCCTL_UID2))) {
					printk(KERN_ERR DEV_NAME ":open() NG UID = 0x%lx\n", uid_chk);
					return -EIO;
				}
			} else {
				printk(KERN_ERR DEV_NAME ":open() NG system_rev error\n");
				return -EIO;
			}
		} else {
			uid_chk = sys_getuid();
...
static int felica_cen_if_close(struct inode *inode, struct file *filp)
{
	int ret = 0;
...
static int felica_cen_if_write(
	struct file *filp, const char __user *data, size_t len, loff_t *pos)
{
	...
	ret = felica_cen_sw_write(localbuf[0]);
	if (unlikely(ret)) {
		return ret;
	}
...
static ssize_t felica_cen_if_read(
...
static struct file_operations driver_fops = {
	.open 		= felica_cen_if_open,
	.read		= felica_cen_if_read,
	.write		= felica_cen_if_write,
	.release 	= felica_cen_if_close,
};

static int __devinit felica_cen_probe(struct platform_device *pdev)
{
...
static int __devexit felica_cen_remove(struct platform_device *pdev)
{
	int ret = 0;
...
static struct platform_device cen_device = {
	.name           = "felica_cen",
	.id             = -1,
};
static struct platform_driver cen_driver = {
	...
	.probe			= felica_cen_probe,
	.remove			= __devexit_p(felica_cen_remove),
};
...
static int felica_cen_init(void)
{
...
#endif
	ret = platform_device_register(&cen_device);
	if (unlikely(ret)) {
	...
	if (unlikely(ret)) {
		platform_device_unregister(&cen_device);
...
MODULE_DESCRIPTION("Felica CEN Port Control Driver");
MODULE_LICENSE("GPL");
