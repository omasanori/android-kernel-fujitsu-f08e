...
#endif
/* FUJITSU:2012-09-28 Stop the double setup of gamma start */
//			(0x400*0) + 0 + i*4,
			(0x400*mdp_lut_i) + 0 + i*4,
/* FUJITSU:2012-09-28 Stop the double setup of gamma end */
				((g & 0xff) |
				 ((b & 0xff) << 8) |
				 ((r & 0xff) << 16)));
...
/* FUJITSU:2012-10-31 DISP Set the double setup of color lookup values start */
static int mdp_lut_hw_update(struct fb_cmap *cmap);
static int mdp_lut_hw_update_backside(struct fb_cmap *cmap)
{
    if (fj_mdp_lut_tablenum_updated || fj_mdp_lut_tablenum < 0) {
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
		mdp_clk_ctrl(1);
...
/* FUJITSU:2012-10-31 DISP Set the double setup of color lookup values end */
static int mdp_lut_hw_update(struct fb_cmap *cmap)
{
	int i;
	u16 *c[3];
...
/* FUJITSU:2012-02-06 GammaCorrection end */
/* FUJITSU:2012-08-06 GammaCorrection start */
//	/*mask off non LUT select bits*/
//	out = inpdw(MDP_BASE + 0x90070);
//	MDP_OUTP(MDP_BASE + 0x90070, (mdp_lut_i << 10) | 0x7 | out);
/* FUJITSU:2012-08-06 GammaCorrection end */
	mdp_clk_ctrl(0);
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
	mdp_lut_i = (mdp_lut_i + 1)%2;
/* FUJITSU:2012-10-31 DISP Set the double setup of color lookup values start */
	mdp_lut_hw_update_backside(cmap);
/* FUJITSU:2012-10-31 DISP Set the double setup of color lookup values end */
...
void fj_mdp_set_lut_table_index(int i, uint16 panel_type)
/* FUJITSU:2012-08-28 DISP support MIPI VIDEO panel end */
	{
	struct fb_cmap cmap;
	unsigned long flags; /* FUJITSU:2012-04-04 DISP change mutex end */
...
//	if (mdp_prim_panel_type == LCDC_PANEL) {
	if (panel_type == LCDC_PANEL || panel_type == MIPI_VIDEO_PANEL) {
/* FUJITSU:2012-08-28 DISP support MIPI VIDEO panel end */
		MDP_OUTP(MDP_BASE + 0x90070, (mdp_lut_i << 10) | 0x17);
		mdp_clk_ctrl(0);
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
...
/* FUJITSU:2012-03-26 DISP support LCDC panel end */
		mdp_clk_ctrl(0);
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
#if 0  /* FUJITSU:2012-04-04 DISP change mutex start */
		mutex_lock(&mdp_lut_push_sem);
#else
		spin_lock_irqsave(&mdp_lut_push_lock, flags);
#endif /* FUJITSU:2012-04-04 DISP change mutex end */
		mdp_lut_push = 1;
		mdp_lut_push_i = mdp_lut_i;
#if 0  /* FUJITSU:2012-04-04 DISP change mutex start */
...
extern void mipi_dsi_panel_shutdown(void);
static bool shutdown_state=false;
static void mdp_shutdown(struct platform_device *pdev)
{
	shutdown_state = true;
	if (pdev->id == 0) {
		pr_info("[LCD]mdp_shutdown power off\n");
		mipi_dsi_panel_shutdown();
	}
	return;
}

bool mdp_shutdown_exec_determ(void)
{
...
#endif
/* FUJITSU:2013-06-28 DISP clear lut operation start */
	mdp_lut_push = 0;
/* FUJITSU:2013-06-28 DISP clear lut operation end */
	pr_debug("%s:-\n", __func__);
	return ret;
}
...
/* FUJITSU:2012-12-17 DISP Turn on LCD at bootloader start */
static int bootlogo_clean = 0;
/* FUJITSU:2012-12-17 DISP Turn on LCD at bootloader end */
...
/* FUJITSU:2012-12-17 DISP Turn on LCD at bootloader start */
	if (bootlogo_clean ==0) {
		MDP_OUTP(MDP_BASE + 0x90008, mfd->fbi->fix.smem_start);
		mdelay(32);
		bootlogo_clean =1;
	}
/* FUJITSU:2012-12-17 DISP Turn on LCD at bootloader end */
...
/* FUJITSU:2013-06-28 DISP clear lut operation start */
	mdp_lut_push = 0;
/* FUJITSU:2013-06-28 DISP clear lut operation end */
/* FUJITSU:2012-09-28 DISP gamma re-setting start */
	if(mdp_rev > MDP_REV_41 && footswitch) {
		if(fj_mdp_lut_tablenum>=0)
			fj_mdp_set_lut_table_index(fj_mdp_lut_tablenum, mfd->panel.type);
	}
/* FUJITSU:2012-09-28 DISP gamma re-setting end */
...
/* FUJITSU:2013-04-12 DISP for MDP underrun start */
	if(mdp_bus_usecases[bus_index].vectors->ab < 684288000 * 2)	/* FUJITSU:2013-05-09 DISP for MDP underrun */
	mdp_bus_usecases[bus_index].vectors->ab = 684288000 * 2;
	if(mdp_bus_usecases[bus_index].vectors->ib < 855360000 * 2)	/* FUJITSU:2013-05-09 DISP for MDP underrun */
	mdp_bus_usecases[bus_index].vectors->ib = 855360000 * 2;
/* FUJITSU:2013-04-12 DISP for MDP underrun end */
...
/* FUJITSU:2012-12-17 DISP add mdp_clk_ctrl start */
        printk("### %s mdp_clk_ctrl(1)\n",__func__);
        mdp_clk_ctrl(1);
/* FUJITSU:2012-12-17 DISP add mdp_clk_ctrl end */
...
#ifdef CONFIG_SECURITY_FJSEC
			fjsec_make_page_acl("splash_screen", mdp_pdata->splash_screen_addr, mdp_pdata->splash_screen_size);
#endif
			cp = (char *)ioremap(
					mdp_pdata->splash_screen_addr,
					mdp_pdata->splash_screen_size);
