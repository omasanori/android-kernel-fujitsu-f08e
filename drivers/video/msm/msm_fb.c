...
/* FUJITSU:2012-08-28 DISP support MIPI VIDEO panel start */
//extern void fj_mdp_set_lut_table_index(int i);
extern void fj_mdp_set_lut_table_index(int i, uint16 panel_type);
/* FUJITSU:2012-08-28 DISP support MIPI VIDEO panel end */
...
/* FUJITSU:2012-02-27_[N]-12-1ST-121-8960-PARTS2-0389_BUGFIX-GEN_ADD-S  */
extern int makercmd_mode;
/* FUJITSU:2012-02-27_[N]-12-1ST-121-8960-PARTS2-0389_BUGFIX-GEN_ADD-E  */
...
ssize_t (*panel_get_wcabc)(char *buf) = NULL;
ssize_t (*panel_set_wcabc)(const char *buf,size_t count) = NULL;
#endif /* FUJITSU:2013-05-17 mode_change add */

/* FUJITSU:2012-03-07 GammaCorrection start */ 
int gamma_condition[]={0,0,0,0,0,0,0};
...
/* FUJITSU:2012-08-28 DISP support MIPI VIDEO panel start */
	...
	if (!fbi) {
		GAMMA_LOG("msm_fb.c: msm_fb_data_type is NULL!!\n");
		return count;
	}
...
/* FUJITSU:2012-08-28 DISP support MIPI VIDEO panel end */
	...
	if (cmd[0] == 'l' && cmd[1] == 'u' && cmd[2] == 't') {
		if (priority[0] == '1' ){
			...
			table=1;
		} else if (priority[0] == '3' || priority[0] == '4') {
			table=2;
	...
	if (cmd[0] == 'o' && cmd[1] == 'n') {
		...
		case '8'://for MHL
			...
			break;
		default:
			GAMMA_LOG("%s: Invalid command is inputted!!!! LINE=%d\n", __func__, __LINE__);
			return count;
		}
		...
		case '8'://for MHL
			...
			break;
		default:
			GAMMA_LOG("%s: Invalid command is inputted!!\n", __func__);
			return count;
		}
	...
	if (gamma_condition[5] == 1) {
		table = 5;
		GAMMA_LOG("Set gammatable for MHL. table=%d\n",table);
	} else if (gamma_condition[4] == 1) {
		table = 4;
		GAMMA_LOG("Set gammatable for privacy view. table=%d\n",table);
	} else if (gamma_condition[3] == 1) {
		table = 3;
		GAMMA_LOG("Set gammatable for BluelightCut view. table=%d\n",table);
	} else if (gamma_condition[2] == 1) {
		table = 2;
		GAMMA_LOG("Set gammatable for libMmp1. table=%d\n",table);
	} else if (gamma_condition[1] == 1) {
		table = 1;
		GAMMA_LOG("Set gammatable for libMmp2. table=%d\n",table);
	} else if (gamma_condition[0] == 1) {
		table = 0;
		GAMMA_LOG("Set gammatable for Outdoor view. table=%d\n",table);
	} else {
		table = -1;
	...
	if (cmd[0] == '0') {
		table = -1;
		GAMMA_LOG("%s:Input was only 1 cmd. Set gammatable for default. table=%d\n", __func__, table);
	} else if (cmd[0] == '1'  ) {
		table = 0;
		GAMMA_LOG("%s:Input was only 1 cmd. Set gammatable for Outdoor view. table=%d\n", __func__, table);
	} else if (cmd[0] == '2') {
		table = 1;
		GAMMA_LOG("%s:Input was only 1 cmd. Set gammatable for libMmp2. table=%d\n", __func__, table);
	} else if (cmd[0] == '3' || cmd[0] == '4') {
		table = 2;
		GAMMA_LOG("%s:Input was only 1 cmd. Set gammatable for libMmp1. table=%d\n", __func__, table);
	} else if (cmd[0] == '5' ) {
		table = 3;
		GAMMA_LOG("%s:Input was only 1 cmd.Set gammatable for BluelightCut view. table=%d\n", __func__, table);
	} else if (cmd[0] == '6') {
		table = 4;
		GAMMA_LOG("%s:Input was only 1 cmd.Set gammatable for privacy view. table=%d\n", __func__, table);
	} else if (cmd[0] == '8') {
		table = 5;
		GAMMA_LOG("%s:Input was only 1 cmd.Set gammatable for MHL. table=%d\n", __func__, table);
	}
	mutex_lock(&msm_fb_ioctl_lut_sem);
/* FUJITSU:2012-08-28 DISP support MIPI VIDEO panel start */
//	fj_mdp_set_lut_table_index(table);
	fj_mdp_set_lut_table_index(table, mfd->panel.type);
/* FUJITSU:2012-08-28 DISP support MIPI VIDEO panel end */
	mutex_unlock(&msm_fb_ioctl_lut_sem);

    if (panel_set_wcabc != NULL) {
        if (gamma_condition[0] == 1) { //FUJITSU:2013/06/18 Change WCABC Trig
            panel_set_wcabc("05",3);
        } else {
            panel_set_wcabc("03",3);
        }
    }
...
/* FUJITSU:2013-07-03 DrawUpdated start */
static int draw_updated;
/* FUJITSU:2013-07-03 DrawUpdated end */
...
/* FUJITSU:2013-07-03 DrawUpdated start */
	if (!mfd->panel_power_on || !( bl_updated || draw_updated )) {
/* FUJITSU:2013-07-03 DrawUpdated end */
...
/* FUJITSU:2013-07-03 DrawUpdated start */
/* FUJITSU:2013-03-25 DISP When only requesting from DISPLAY_1, bl_updated is set to 0. start */
			if (mfd->panel_info.pdest == DISPLAY_1) {
/* FUJITSU:2013-03-25 DISP When only requesting from DISPLAY_1, bl_updated is set to 0. end */
				bl_updated = 0;
				draw_updated = 0;
			}
/* FUJITSU:2013-07-03 DrawUpdated end */
...
#if 0  /* FUJITSU:2012-11-19 DISP add LCD actual size start */
	var->height = -1,	/* height of picture in mm */
	var->width = -1,	/* width of picture in mm */
#endif /* FUJITSU:2012-11-19 DISP add LCD actual size end */
	var->accel_flags = 0,	/* acceleration flags */
	var->sync = 0,	/* see FB_SYNC_* */
	var->rotate = 0,	/* angle we rotate counter clockwise */
	mfd->op_enable = FALSE;
/* FUJITSU:2012-11-19 DISP add LCD actual size start */
...
#if 1  //FUJITSU:2013/01/10 disable detachRGB from HWC_init  TODO:check nextUpdate
    if ( makercmd_mode && (req.src.format == MDP_RGB_BORDERFILL) ) {
        printk("## %s format change BF to RGB\n",__func__);
        req.src.format = MDP_RGB_888;
    }
#endif //FUJITSU:2013/01/10 disable 

	ret = mdp4_overlay_set(info, &req);
	if (ret) {
/* FUJITSU:2013-05-09 DISP for MDP underrun start */
//		printk(KERN_ERR "%s: ioctl failed, rc=%d\n",
		pr_debug( "%s: ioctl failed, rc=%d\n",
/* FUJITSU:2013-05-09 DISP for MDP underrun end */
			__func__, ret);
		return ret;
	}
