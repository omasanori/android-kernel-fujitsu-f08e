...
/* FUJITSU:2013-03-05 ExternalDisplay Disable external video start */
static int fj_external_video_output_ctrl = 0;
int fj_external_video_output_state = 0;
module_param(fj_external_video_output_ctrl, int, 0664);
/* FUJITSU:2013-04-12 ExternalDisplay Disable external video HDMI start */
static int fj_external_video_output_ctrl_hdmi = 0;
module_param(fj_external_video_output_ctrl_hdmi, int, 0664);
static int fj_external_video_output_forbidden = 0;
module_param(fj_external_video_output_forbidden, int, 0660);
/* FUJITSU:2013-04-12 ExternalDisplay Disable external video HDMI start */
/* FUJITSU:2013-03-05 ExternalDisplay Disable external video end   */
/* FUJITSU:2013-03-14 ExternalDisplay Disable external video start */
static int fj_wfd_screen_status = 0;
static int fj_wfd_audio_status = 0;
static int fj_wfd_screen_secure_status = 0;
static int fj_wfd_audio_secure_status = 0;
module_param(fj_wfd_screen_status, int, 0664);
module_param(fj_wfd_audio_status, int, 0664);
module_param(fj_wfd_screen_secure_status, int, 0664);
module_param(fj_wfd_audio_secure_status, int, 0664);
/* FUJITSU:2013-03-14 ExternalDisplay Disable external video end   */
	...
	*srcp_ihdl = ion_import_dma_buf(display_iclient, mem_id);
	...
	if (iom->prev_ihdl[plane]) {
		mdp4_overlay_iommu_2freelist(pipe->mixer_num,
						iom->prev_ihdl[plane]);
		mdp4_stat.iommu_drop++;
		pr_debug("%s: dropped, ndx=%d plane=%d\n", __func__,
						pipe->pipe_ndx, plane);
	}
...
/* FUJITSU:2013-04-25 DISP for MDP underrun start */
//	return (u32)rst;
	return ((u32)rst>mdp_max_clk) ? (u32)rst : mdp_max_clk;
/* FUJITSU:2013-04-25 DISP for MDP underrun end */
}
				  ...
				  int flag)
{
	...
	if (flag) {
		...
		if ((perf_req->mdp_ab_bw > perf_cur->mdp_ab_bw) ||
			...
			if(perf_req->mdp_ab_bw!=0)	/* FUJITSU:2013-3-14 TEMP fix skip MDP_BUS off */
...
//FUJITSU 2013.05.14  START
	if(mfd->sec_mapped) {
		mfd->sec_active = FALSE;
		mdp4_unmap_sec_resource(mfd);
	}
//FUJITSU 2013.05.14  END
	mdp4_overlay_pipe_free(pipe);
