...
/* FUJITSU:2013-01-18 sddl_encrypt add start */
#if 1
	#undef PDEBUG
	#define PDEBUG(fmt, args...) printk(KERN_ERR "%s(%i, %s): " fmt "\n", \
	...
	#undef PWARN
	#define PWARN(fmt, args...) printk(KERN_ERR "%s(%i, %s): " fmt "\n", \
			__func__, current->pid, current->comm, ## args)
...
retry:
		...
		if(i>0)
		{
			pr_warning("Interrupted: exiting send_cmd loop cnt[%d]\n",i);
		}
		if (data->abort) {
...
/* FUJITSU:2013-01-18 sddl_encrypt add start */
static int qseecom_send_oem_cmd(struct qseecom_dev_handle *data, void __user *argp)
{
	int ret = 0;
	struct qseecom_send_cmd_oem_req req;

	u32 scm_ret = 0;
	u8 *mdata_buf = NULL;
	u8 *work_buf = NULL;
	u8 *iv_buf = NULL;
/* FUJITSU:2013-05-14 sddl_encrypt add start */
	u8 *ret_buf = NULL;
	int ret_buf_size = 5;
/* FUJITSU:2013-05-14 sddl_encrypt add end */
	struct oem_req {
		u32	target;
		u32	image_addr;
		u32	image_len;
		u32	buff_addr;
		u32	iv_addr;
		u32	ret_addr;			/* FUJITSU:2013-05-14 sddl_encrypt add */
	} request;
	...
	if (req.data_buf == NULL || req.data_len <= 0) {
...
/* FUJITSU:2013-05-14 sddl_encrypt add start */
	if (req.param3 == NULL) {
		PERR("ret buffer is null");
		ret = -EINVAL;
		goto err;
	}
/* FUJITSU:2013-05-14 sddl_encrypt add end */

	mdata_buf = kmemdup(req.data_buf, req.data_len, GFP_KERNEL);
	if (!mdata_buf){
		PERR("kmemdup() buff memory malloc ERROR!!!!");
		ret = -ENOMEM;
		goto err;
	}
	work_buf = kmalloc(req.data_len, GFP_KERNEL);
	if (!work_buf){
		PERR("kmalloc() work memory malloc ERROR!!!!");
		ret = -ENOMEM;
		goto err;
	}
/* FUJITSU:2013-05-14 sddl_encrypt add start */
	ret_buf = kmalloc(ret_buf_size, GFP_KERNEL);
	if (!ret_buf){
		PERR("kmalloc() ret memory malloc ERROR!!!!");
		ret = -ENOMEM;
		goto err;
	}
/* FUJITSU:2013-05-14 sddl_encrypt add end */
	...
	if (req.param2 != NULL) {
		iv_buf = kmemdup(req.param2, TZ_OEM_IV_SIZE, GFP_KERNEL);
		if (!iv_buf){
			PERR("kmemdup() iv memory malloc ERROR!!!!");
			ret = -ENOMEM;
			goto err;
		}
	}

	request.target = req.param1;
	request.image_addr = virt_to_phys(mdata_buf);
	request.image_len = req.data_len;
	request.buff_addr = virt_to_phys(work_buf);
	request.ret_addr = virt_to_phys(ret_buf);			/* FUJITSU:2013-05-14 sddl_encrypt add */

	if (req.param2 != NULL) {
		request.iv_addr = virt_to_phys(iv_buf);
	} else {
		request.iv_addr = 0;
	}

	ret = scm_call(SCM_SVC_OEM, req.cmd_id, &request, sizeof(request), &scm_ret, sizeof(scm_ret));
	if (ret) {
		PERR("qseecom_scm_call failed with err: %d\n", ret);
		goto err;
	}

	memcpy(req.data_buf, work_buf, req.data_len);
	memcpy(req.param3, ret_buf, ret_buf_size);			/* FUJITSU:2013-05-14 sddl_encrypt add */
...
/* FUJITSU:2013-01-18 sddl_encrypt add start */
	case QSEECOM_IOCTL_SEND_OEM_CMD_REQ: {
		/* Only one client allowed here at a time */
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
		ret = qseecom_send_oem_cmd(data, argp);
		atomic_dec(&data->ioctl_count);
		wake_up_interruptible(&data->abort_wq);
		mutex_unlock(&app_access_lock);
		if (ret)
			PERR("failed qseecom_send_cmd: %d", ret);
		break;
	}
/* FUJITSU:2013-01-18 sddl_encrypt add end */
