...
/* FUJITSU:2012-09-25 SD add start */
#ifdef CONFIG_MMC_PASSWORDS
    #define MSM_MMC_TX_RX_INACTIVE_DELAY_TIME    (1000*1000)/* us */
#else
    #define MSM_MMC_TX_RX_INACTIVE_DELAY_TIME    (1000)     /* us */
#endif
/* FUJITSU:2012-09-25 SD add end */

/* FUJITSU:2013-02-05 SD mod start */
...
/* FUJITSU:2013-02-05 SD mod end */
/* FUJITSU:2013-02-08 SD mod start */
#define MSM_MMC_VDD_DATA_OFF_TIME	1000 /* msecs */
/* FUJITSU:2013-02-08 SD mod end */

/* FUJITSU:2013-05-07 SD add start */
#define	SD2_BLOCK_DEFERRED_RESUME	/* deferred resume for insert & remove interrupt */
/* FUJITSU:2013-05-07 SD add end */
...
/* FUJITSU:2013-02-08 SD mod start */
extern int check_device_id_sd(int id);
/* FUJITSU:2013-02-08 SD mod end */
/* FUJITSU:2013-04-22 sdcc mod start */
#if 1
static unsigned int msmsdcc_pwrsave = 1;
#else
static unsigned int msmsdcc_pwrsave = 0;
#endif
/* FUJITSU:2013-04-22 sdcc mod end */
...
static void msmsdcc_reset_dpsm(struct msmsdcc_host *host)
{
	...
	if (is_wait_for_tx_rx_active(host)) {
		...
		while (readl_relaxed(host->base + MMCISTATUS) &
			 ...
			 * still active.
			 */
/* FUJITSU:2012-09-25 SD mod start */
/*			if (ktime_to_us(ktime_sub(ktime_get(), start)) > 1000) { */
			if (ktime_to_us(ktime_sub(ktime_get(), start)) > MSM_MMC_TX_RX_INACTIVE_DELAY_TIME) {
/* FUJITSU:2012-09-25 SD mod emd */
...
/* FUJITSU:2013-02-08 SD mod start */
	if (check_device_id_sd(host->pdev_id) == true) {
		if ((enable) && (!curr_slot->vdd_data->is_enabled)) {
			getnstimeofday(&now_ts);
			end_ts = host->vdd_off_start_ts;
			...
			pr_debug("%s: vdd_data off->on. start_ts=%ld.%ld now_ts=%ld.%ld end_ts=%ld.%ld\n",
				mmc_hostname(host->mmc),
				host->vdd_off_start_ts.tv_sec, host->vdd_off_start_ts.tv_nsec,
				now_ts.tv_sec, now_ts.tv_nsec,
				end_ts.tv_sec, end_ts.tv_nsec);

			if ((host->vdd_off_start_ts.tv_sec == 0) && (host->vdd_off_start_ts.tv_nsec == 0)) { /* init on? */
				pr_debug("%s: init skip\n", mmc_hostname(host->mmc));
			} else if (timespec_compare(&host->vdd_off_start_ts, &now_ts) > 0) { /* future time? */
				...
				pr_debug("%s: wait start. sleep_ts=%ld.%ld sleep_ms=%d\n",
					mmc_hostname(host->mmc), sleep_ts.tv_sec, sleep_ts.tv_nsec, sleep_ms);
				msleep(sleep_ms);
				pr_debug("%s: wait end\n", mmc_hostname(host->mmc));
			} else { /* time over? */
				pr_debug("%s: non wait\n", mmc_hostname(host->mmc));
			...
			getnstimeofday(&host->vdd_off_start_ts);
			pr_debug("%s: vdd_data on->off. start_ts=%ld.%ld\n",
				mmc_hostname(host->mmc), host->vdd_off_start_ts.tv_sec, host->vdd_off_start_ts.tv_nsec);
		}
	}
/* FUJITSU:2013-02-08 SD mod end */
...
msmsdcc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	/* FUJITSU:2013-04-05 Power save feature disable mod start */
	if (msmsdcc_is_pwrsave(host)) {
		clk |= MCI_CLK_PWRSAVE;
	} else {
		clk &= ~MCI_CLK_PWRSAVE;
	}
	/* FUJITSU:2013-04-05 Power save feature disable mod end */
...
static void status_work_handler(struct work_struct *work)
{
		...
		if (status == oldstatus) {
			...
			if (mach_count >= (MSM_MMC_STATUS_MACH_COUNT - 1)) {
				pr_info("%s: %d consecutive read. read_count=%d slot_status=%d\n",
					mmc_hostname(host->mmc), mach_count, i + 1, status);
				break;
			}
		} else {
			mach_count = 0;
	...
	if (i >= MSM_MMC_STATUS_RETRY) {
		pr_info("%s: non consecutive read. read_count=%d slot_status=%d\n",
			mmc_hostname(host->mmc), i, status);
		status = 1;
	}
	...
	host->status_working_flg = 0;
	wake_unlock(&host->status_wlock);
	pr_info("%s: %s: leave\n", mmc_hostname(host->mmc), __func__);
}
/* FUJITSU:2013-02-05 SD mod end */
...
/*	msmsdcc_check_status((unsigned long) host);*/
	if (host->status_working_flg == 0) {
		host->status_working_flg = 1;
		wake_lock(&host->status_wlock);
		queue_delayed_work(host->status_workqueue, &host->status_work,
			msecs_to_jiffies(MSM_MMC_STATUS_DELAY));
	} else {
...
/* FUJITSU:2013-02-08 SD mod start */
	 host->vdd_off_start_ts.tv_sec = 0;
	 host->vdd_off_start_ts.tv_nsec = 0;
/* FUJITSU:2013-02-08 SD mod end */
...
/* FUJITSU:2013-05-07 SD add start */
#ifdef SD2_BLOCK_DEFERRED_RESUME
	if (plat->status_irq) {
		ret = irq_set_irq_wake(plat->status_irq, 1);
		if (ret) {
			pr_err("%s: %s: failed irq_set_irq_wake  irq=%d ret=%d\n", mmc_hostname(mmc), __func__, plat->status_irq, ret);
			goto sdiowakeup_irq_free;
		}
	}
#endif
...
/* FUJITSU:2013-02-05 SD mod start */
	if (plat->status_irq) {
		host->status_workqueue = create_singlethread_workqueue("msmsdcc_status");
		if (!host->status_workqueue) {
			pr_err("%s: create status workqueue failed\n", __func__);
			goto remove_polling_file;
		}
		INIT_DELAYED_WORK(&host->status_work, status_work_handler);
		host->status_working_flg = 0;
		pr_info("%s: create status workqueue done\n", mmc_hostname(mmc));
		wake_lock_init(&host->status_wlock, WAKE_LOCK_SUSPEND,
				mmc_hostname(mmc));
	}
/* FUJITSU:2013-02-05 SD mod end */
	return 0;
...
/* FUJITSU:2013-02-05 SD mod end */
		free_irq(plat->status_irq, host);
/* FUJITSU:2013-02-05 SD mod start */
		wake_lock_destroy(&host->status_wlock);
