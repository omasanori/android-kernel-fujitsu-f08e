...
/* FUJITSU:2012/12/17 security add start */
static int pm8xxx_lateinit(struct device *dev, bool doit)
{
	int rc;
	int ret;
	u8 ctrl_reg;
...
static ssize_t pm8xxx_rtc_sysfs_show_lateinit1(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;

	ret = pm8xxx_lateinit(dev, false);
	if (ret < 0) {
		return 0;
	}
	buf[0] = ret;
...
static ssize_t pm8xxx_rtc_sysfs_show_lateinit2(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret;

	ret = pm8xxx_lateinit(dev, true);
	if (ret < 0) {
		return 0;
	}
	buf[0] = ret;
...
static struct device_attribute dev_attr_lateinit1 = __ATTR(lateinit1, 0440, pm8xxx_rtc_sysfs_show_lateinit1, NULL);
static struct device_attribute dev_attr_lateinit2 = __ATTR(lateinit2, 0440, pm8xxx_rtc_sysfs_show_lateinit2, NULL);
/* FUJITSU:2012/12/17 security add end */
...
static int __devinit pm8xxx_rtc_probe(struct platform_device *pdev)
{
	...
	/* FUJITSU:2012/12/17 security del start */
	/*
	if (!(ctrl_reg & PM8xxx_RTC_ENABLE)) {
		ctrl_reg |= PM8xxx_RTC_ENABLE;
		rc = pm8xxx_write_wrapper(rtc_dd, &ctrl_reg, rtc_dd->rtc_base,
	...
	*/
	/* FUJITSU:2012/12/17 security del end */
	...
	/* FUJITSU:2012/12/17 security add start */
	rc = device_create_file(&pdev->dev, &dev_attr_lateinit1);
	if (rc)
		dev_err(&pdev->dev, "failed to create lateinit1 attribute, %d\n", rc);

	rc = device_create_file(&pdev->dev, &dev_attr_lateinit2);
	if (rc)
		dev_err(&pdev->dev, "failed to create lateinit2 attribute, %d\n", rc);
	/* FUJITSU:2012/12/17 security add end */
...
fail_req_irq:
	/* FUJITSU:2012/12/17 security add start */
	device_remove_file(&pdev->dev, &dev_attr_lateinit1);
	device_remove_file(&pdev->dev, &dev_attr_lateinit2);
	/* FUJITSU:2012/12/17 security add end */
