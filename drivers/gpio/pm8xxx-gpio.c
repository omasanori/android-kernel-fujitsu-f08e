...
/* FUJITSU:2013-01-26 GPIO Dump add start   */
void pm8xxx_gpio_dbg_out(int detailflag)
{
	static const char * const cmode[] = { "I", "IO", "O", "OFF" };
	...
	list_for_each_entry(pm_gpio_chip, &pm_gpio_chips, link) {
		...
		//: check parameter.
		if (gpio_chip == NULL) {
			printk("pm8xxx_gpio_dbg_out error return\n");
			continue;
		}
		dev = gpio_chip->dev;
		...
		printk("PMICGPIO,gpio,alternate,direction,value,flags,voltage,pull,strength\n");
		for (i = 0; i < gpio_chip->ngpio; i++) {
			label = gpiochip_is_requested(gpio_chip, i);
			mode = (pm_gpio_chip->bank1[i] & PM_GPIO_MODE_MASK) >> PM_GPIO_MODE_SHIFT;
			state = pm_gpio_get(pm_gpio_chip, i);

			for (j = 0; j < PM_GPIO_BANKS; j++) {
				bank[j] = j << PM_GPIO_BANK_SHIFT;
				pm8xxx_writeb(gpio_chip->dev->parent,
					SSBI_REG_ADDR_GPIO(i),bank[j]);
				pm8xxx_readb(gpio_chip->dev->parent,
					SSBI_REG_ADDR_GPIO(i),&(bank[j]));
			}

			printk("PMICGPIO,%d,%d,%s,%s,",
				 1 + i,
 				(bank[4] >> 1) & 0x07,
				cmode[mode],
				state ? "H" : "L");
...
EXPORT_SYMBOL(pm8xxx_gpio_dbg_out);
/* FUJITSU:2013-01-26 GPIO Dump add end   */
