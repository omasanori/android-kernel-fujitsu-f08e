...
/* FUJITSU:2012-11-16 GPIO Dump add start   */
unsigned __msm_gpio_get_config(unsigned gpio)
{
	return __raw_readl(GPIO_CONFIG(gpio));
}

unsigned __msm_gpio_get_oe_config(unsigned gpio)
{
	return __raw_readl(GPIO_CONFIG(gpio)) & BIT(GPIO_OE_BIT);
}
unsigned __msm_gpio_get_io_value(unsigned gpio)
{
	return __raw_readl(GPIO_IN_OUT(gpio));
}
/* FUJITSU:2012-11-16 GPIO Dump add end   */
