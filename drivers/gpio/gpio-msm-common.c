...
#define APQ8064GPIO_MAX 89
void msm_gpio_dbg_out(void)
{
	unsigned		i;
	...
	struct device		*dev = msm_gpio.gpio_chip.dev;
	...
	printk("APQGPIO,gpio,alternate,direction,value(out),value(in),config,pull,strength\n"); 
	for (i = 0; i < chip->ngpio && gpio <= APQ8064GPIO_MAX; i++, gpio++) {
		spin_lock_irqsave(&tlmm_lock, irq_flags);
		//: get config value.
		config = __msm_gpio_get_config(gpio);
		//: check in/out bit.
		is_out = __msm_gpio_get_oe_config(gpio);
		//: get gpio value.
		state = __msm_gpio_get_io_value(gpio);
		spin_unlock_irqrestore(&tlmm_lock, irq_flags);

		printk("APQGPIO,%d,%d,%s,%s,%s,\"%04x\",", 
					gpio, 
					(config>>2)&0x0f,
					is_out ? "O" : "I",
					(state & 2) ? "H" : "L", 
...
EXPORT_SYMBOL(msm_gpio_dbg_out);
