...
/* FUJITSU:2013-1-28 GPIO Dump add start   */
void pm8xxx_mpp_dbg_out(int detailflag)
{
	...
	list_for_each_entry(mpp_chip, &pm8xxx_mpp_chips, link) {
		chip = &mpp_chip->gpio_chip;
		
		if (chip == NULL) {
			printk("pm8xxx_mpp_dbg_out error return\n");
			continue;
		}
		dev = chip->dev;

		printk("MPP_GPIOs %d-%d, %s/%s, %s, %s:\n",
		...
		printk("MPPGPIO,no,value,direction\n"); 
		for (i = 0; i < mpp_chip->nmpps; i++) {
			...
			printk("MPPGPIO,%d,%s,%s,", 
							1 + i, 
							state ? "H" : "L",
							ctype[type]);
			if (detailflag == 1)
				printk("\"%02x\"", mpp_chip->ctrl_reg[i]);
			printk("\n");
		}
	}
}
EXPORT_SYMBOL(pm8xxx_mpp_dbg_out);
/* FUJITSU:2013-1-28 GPIO Dump add end   */
