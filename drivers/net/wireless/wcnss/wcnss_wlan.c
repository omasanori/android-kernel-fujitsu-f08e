...
static int _fj_wlan_fem_init(void)
{
	int rc;

	printk(KERN_INFO "%s: begin\n", __func__);

	rc = gpio_request(44, "wlan_fem_on");
	if (rc < 0) {
		printk(KERN_ERR "%s: gpio_request error:%d\n", __func__, rc);
		return rc;
	}
	rc = gpio_tlmm_config(GPIO_CFG(44, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		printk(KERN_ERR "%s: gpio_tlmm_config error:%d\n", __func__, rc);
		return rc;
	}
    rc = gpio_direction_output(44, 0);
	if (rc < 0) {
		printk(KERN_ERR "%s: gpio_direction_output error:%d\n", __func__, rc);
		return rc;
	}
...
static int _fj_wlan_fem_power(int on)
{
	int rc;

	printk(KERN_INFO "%s(on:%d): begin\n", __func__, on);

	if (on != 0) {
	    rc = gpio_direction_output(44, 1);
		if (rc < 0) {
			printk(KERN_ERR "%s: gpio_direction_output error:%d\n", __func__, rc);
			return rc;
		}
...
{
	return _fj_wlan_fem_power(1);
}
EXPORT_SYMBOL(fj_wlan_fem_power_on);

int fj_wlan_fem_power_off(void)
{
	return _fj_wlan_fem_power(0);
}
EXPORT_SYMBOL(fj_wlan_fem_power_off);
...
/* FUJITSU:2013-02-19 add start */
	_fj_wlan_fem_init();
/* FUJITSU:2013-02-19 add end */
