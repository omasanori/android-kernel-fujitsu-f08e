...
/* FUJITSU:2013_04_09 mod camera start */
#include <linux/delay.h>
/* FUJITSU:2013_04_09 mod camera end */
#include <linux/of.h>
				...
				uint32_t id, struct msm_ctrl_cmd *out)
{
	...
	do {
		...
		D("%s: wait_event interrupted by signal, remain_count = %d",
			__func__, wait_count);
/* FUJITSU:2013_04_11 mod camera start */
//	} while (1);
	} while (wait_count > 0);
/* FUJITSU:2013_04_11 mod camera end */
	...
	if (list_empty_careful(&queue->list)) {
		...
		if (rc < 0) {
			if (++server_dev->server_evt_id == 0)
				server_dev->server_evt_id++;
/* FUJITSU:2013_04_09 add camera start */
			if(rc == -ERESTARTSYS){
				if(out->type == MSM_V4L2_CLOSE){
					pr_err("%s: 500ms sleep\n", __func__);
					msleep(500);
				} else {
					pr_err("%s: wait_event error %d type %d\n", __func__, rc, out->type);
					return rc;
				}
			} else {
/* FUJITSU:2013_04_09 add camera end */
			pr_err("%s: wait_event error %d\n", __func__, rc);
			return rc;
/* FUJITSU:2013_04_11 add camera start */
			}
/* FUJITSU:2013_04_11 add camera end */
		}
	}
