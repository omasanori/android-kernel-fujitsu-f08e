...
#define PLATFORM_DRIVER_NAME  "m9mo_iu081f2f"
#define m9mo_iu081f2f_obj m9mo_iu081f2f_##obj

#define LOGI(fmt, args...)      printk(KERN_INFO "m9mo_iu081f2f: " fmt, ##args)
//#define LOGI(fmt, args...)      do{}while(0)
//#define LOGD(fmt, args...)      printk(KERN_INFO "m9mo_iu081f2f: " fmt, ##args)
#define LOGD(fmt, args...)      do{}while(0)
#define LOGE(fmt, args...)      printk(KERN_ERR "m9mo_iu081f2f: " fmt, ##args)
...
/*===================================================================*
    LOCAL DECLARATIONS
 *===================================================================*/
DEFINE_MUTEX(m9mo_iu081f2f_mtx);
static struct msm_sensor_ctrl_t m9mo_iu081f2f_s_ctrl;
...
static int m9mo_iu081f2f_i2c_write_isp(
...
static int m9mo_iu081f2f_i2c_read_isp(
	struct i2c_client* client, struct camsensor_i2c_command_isp* cmd)
{
	int rc = 0;
	if (is_use_i2c_driver) {
...
static int m9mo_iu081f2f_i2c_write(
...
static int m9mo_iu081f2f_i2c_read(
	struct i2c_client* client, struct camsensor_i2c_command* cmd)
{
	int rc = 0;
	if (is_use_i2c_driver) {
...
static int32_t m9mo_iu081f2f_get_temp(struct msm_sensor_ctrl_t *s_ctrl,
			uint32_t* temp)
{
    int rc = 0;
...
static int32_t m9mo_iu081f2f_command(struct msm_sensor_ctrl_t *s_ctrl,
			struct cfg_cmd_ctrl_isp* cmd)
{
	...
	if (!cmd->rxlen) {
    	rc = m9mo_iu081f2f_i2c_write_isp(s_ctrl->sensor_i2c_client->client, &i2c_cmd);
	} else {
    	rc = m9mo_iu081f2f_i2c_read_isp(s_ctrl->sensor_i2c_client->client, &i2c_cmd);
		if (!rc) {
...
static int32_t m9mo_iu081f2f_command_normal(struct msm_sensor_ctrl_t *s_ctrl,
			struct cfg_cmd_ctrl* cmd)
{
	...
	if (!cmd->rxlen) {
    	rc = m9mo_iu081f2f_i2c_write(s_ctrl->sensor_i2c_client->client, &i2c_cmd);
	} else {
    	rc = m9mo_iu081f2f_i2c_read(s_ctrl->sensor_i2c_client->client, &i2c_cmd);
		if (!rc) {
			for (cpl=0; cpl<cmd->rxlen; cpl++) {
				cmd->rvalue[cpl] = i2c_cmd.prdata[cpl];
			}
...
static int32_t m9mo_iu081f2f_sensor_setting(struct msm_sensor_ctrl_t *s_ctrl,
			int update_type, int res)
{
    int32_t rc = 0;
//	uint32_t p_clk = 228570000;
//	uint32_t p_clk = 300000000;
	uint32_t p_clk = 265846000;
...
static int32_t m9mo_iu081f2f_set_sensor_mode(struct msm_sensor_ctrl_t *s_ctrl,
	int mode, int res)
{
    int32_t rc = 0;
    LOGI("+%s\n", __func__);
...
static int32_t m9mo_iu081f2f_sensor_mode_init(struct msm_sensor_ctrl_t *s_ctrl,
...
static int32_t m9mo_iu081f2f_sensor_config(struct msm_sensor_ctrl_t *s_ctrl,
	void __user *argp)
{
    ...
    case CFG_GET_TEMP:
        LOGD("-%s (CFG_SENSOR_INIT)!\n", __func__);
    	rc = m9mo_iu081f2f_get_temp(s_ctrl, &cfg.cfg.temp);
        if (rc < 0) {
    		break;
    	}
    ...
    case CFG_COMMAND:
        LOGD("-%s (CFG_COMMAND)\n", __func__);
    	rc = m9mo_iu081f2f_command(s_ctrl, &cfg.cfg.cmd_isp);
    	if (!rc) {
    ...
    case CFG_COMMAND_NORMAL:
    	...
    	rc = m9mo_iu081f2f_command_normal(s_ctrl, &cfg.cfg.cmd);
    	if (!rc) {
    		if (copy_to_user((void *)argp, &cfg, sizeof(struct sensor_cfg_data))) {
                rc = -EFAULT;
    		}
...
static struct bu1852gxw_int bu1852gxw_cam_gpios[] = {
																//1:VREG_S8(+1.2V_M9MO_CORE_A_KO)
	{"+EXP1_GPIO18", BU1852_EXP1_GPIO_TO_SYS(18), false, 1, 2}, //2:GPIO-EXP–GPIO[18](1.8V_OCAM)
	{"+EXP1_GPIO17", BU1852_EXP1_GPIO_TO_SYS(17), false, 1, 2}, //3:GPIO-EXP–GPIO[17](+2.8V_OCAMA)
	{"+EXP1_GPIO19", BU1852_EXP1_GPIO_TO_SYS(19), false, 1, 2}  //4:GPIO-EXP–GPIO[19](+2.7V_OCAM)
};
...
///////////
static int32_t m9mo_iu081f2f_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	int i,rc;
...
static int32_t m9mo_iu081f2f_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	int i;
	LOGI("+%s\n", __func__);
...
static const struct i2c_device_id m9mo_iu081f2f_i2c_id[] = {
	{PLATFORM_DRIVER_NAME, (kernel_ulong_t)&m9mo_iu081f2f_s_ctrl},
	{ },
};

int32_t m9mo_iu081f2f_i2c_probe(struct i2c_client *client,
...
static struct i2c_driver m9mo_iu081f2f_i2c_driver = {
	.id_table = m9mo_iu081f2f_i2c_id,
	.probe  = m9mo_iu081f2f_i2c_probe,
//	.suspend = m9mo_suspend,
//	.resume = m9mo_resume,
	.remove = __exit_p(m9mo_iu081f2f_i2c_remove),
	.driver = {
		.name = PLATFORM_DRIVER_NAME,
	},
};

static struct msm_camera_i2c_client m9mo_iu081f2f_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
};

static const struct of_device_id m9mo_iu081f2f_dt_match[] = {
	{.compatible = "qcom,m9mo_iu081f2f", .data = &m9mo_iu081f2f_s_ctrl},
	{}
};

MODULE_DEVICE_TABLE(of, m9mo_iu081f2f_dt_match);

static struct platform_driver m9mo_iu081f2f_platform_driver = {
	.driver = {
		.name = "qcom,m9mo_iu081f2f",
		.owner = THIS_MODULE,
		.of_match_table = m9mo_iu081f2f_dt_match,
	},
};

static int32_t m9mo_iu081f2f_platform_probe(struct platform_device *pdev)
{
	int32_t rc = 0;
	const struct of_device_id *match;
	match = of_match_device(m9mo_iu081f2f_dt_match, &pdev->dev);
	rc = msm_sensor_platform_probe(pdev, match->data);
	return rc;
}

static int __init m9mo_iu081f2f_init(void)
{
	int rc = 0;
	int i = 0;
	LOGI("+%s\n", __func__);

	rc = platform_driver_probe(&m9mo_iu081f2f_platform_driver,
		m9mo_iu081f2f_platform_probe);
	if (!rc){
...
//		return rc;
	...
	if (is_use_i2c_driver) {
		rc = i2c_add_driver(&m9mo_iu081f2f_i2c_driver);
		if (rc < 0) {
			rc = -ENOTSUPP;
			LOGE("%s: I2C add driver failed\n", __func__);
			i2c_del_driver(&m9mo_iu081f2f_i2c_driver);
//			return rc;
		}
	}
	for (i = 0; i < BU1852GXW_CAM_GPIOS_VAL; i++) {
...
static void __exit m9mo_iu081f2f_exit(void)
{
	if (m9mo_iu081f2f_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&m9mo_iu081f2f_s_ctrl);
		platform_driver_unregister(&m9mo_iu081f2f_platform_driver);
	} else
		i2c_del_driver(&m9mo_iu081f2f_i2c_driver);
	return;
}

int32_t m9mo_iu081f2f_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	return 0; // nop
}

static struct msm_sensor_fn_t m9mo_iu081f2f_func_tbl = {
...
//	.sensor_write_snapshot_exp_gain = msm_sensor_write_exp_gain1,
	.sensor_setting = m9mo_iu081f2f_sensor_setting,
	.sensor_set_sensor_mode = m9mo_iu081f2f_set_sensor_mode,
	.sensor_mode_init = m9mo_iu081f2f_sensor_mode_init,
	.sensor_config = m9mo_iu081f2f_sensor_config,
	.sensor_power_up = m9mo_iu081f2f_sensor_power_up,
	.sensor_power_down = m9mo_iu081f2f_sensor_power_down,
	.sensor_get_csi_params = msm_sensor_get_csi_params,
	.sensor_match_id = m9mo_iu081f2f_sensor_match_id,
};

static struct msm_sensor_reg_t m9mo_iu081f2f_regs = {
	.default_data_type = MSM_CAMERA_I2C_BYTE_DATA,
//	.start_stream_conf = m9mo_iu081f2f_start_settings,
//	.start_stream_conf_size = ARRAY_SIZE(m9mo_iu081f2f_start_settings),
//	.stop_stream_conf = m9mo_iu081f2f_stop_settings,
//	.stop_stream_conf_size = ARRAY_SIZE(m9mo_iu081f2f_stop_settings),
//	.group_hold_on_conf = m9mo_iu081f2f_groupon_settings,
//	.group_hold_on_conf_size = ARRAY_SIZE(m9mo_iu081f2f_groupon_settings),
//	.group_hold_off_conf = m9mo_iu081f2f_groupoff_settings,
//	.group_hold_off_conf_size =
//		ARRAY_SIZE(m9mo_iu081f2f_groupoff_settings),
//	.init_settings = &m9mo_iu081f2f_init_conf[0],
//	.init_size = ARRAY_SIZE(m9mo_iu081f2f_init_conf),
//	.mode_settings = &m9mo_iu081f2f_confs[0],
//	.output_settings = &m9mo_iu081f2f_dimensions[0],
//	.num_conf = ARRAY_SIZE(m9mo_iu081f2f_confs),
};

static long m9mo_iu081f2f_sensor_subdev_ioctl(struct v4l2_subdev *sd,
...
//static struct v4l2_subdev_core_ops m9mo_iu081f2f_subdev_core_ops;
static struct v4l2_subdev_core_ops m9mo_iu081f2f_subdev_core_ops = {
	.ioctl = m9mo_iu081f2f_sensor_subdev_ioctl,
	.s_power = msm_sensor_power,
};

static struct v4l2_subdev_video_ops m9mo_iu081f2f_subdev_video_ops = {
	.enum_mbus_fmt = msm_sensor_v4l2_enum_fmt,
};

static struct v4l2_subdev_ops m9mo_iu081f2f_subdev_ops = {
	.core = &m9mo_iu081f2f_subdev_core_ops,
	.video  = &m9mo_iu081f2f_subdev_video_ops,
};

static struct v4l2_subdev_info m9mo_iu081f2f_subdev_info[] = {
	{
...
static struct msm_sensor_ctrl_t m9mo_iu081f2f_s_ctrl = {
	.msm_sensor_reg = &m9mo_iu081f2f_regs,
	.sensor_i2c_client = &m9mo_iu081f2f_sensor_i2c_client,
	.sensor_i2c_addr = 0x1F,
	.vreg_seq = NULL,
	.num_vreg_seq = 0,
//	.sensor_output_reg_addr = &m9mo_iu081f2f_reg_addr,
//	.sensor_id_info = &m9mo_iu081f2f_id_info,
//	.sensor_exp_gain_info = &m9mo_iu081f2f_exp_gain_info,
	.cam_mode = MSM_SENSOR_MODE_INVALID,
	.wait_num_frames = 0,
	.min_delay = 0,
	.msm_sensor_mutex = &m9mo_iu081f2f_mtx,
	.sensor_i2c_driver = &m9mo_iu081f2f_i2c_driver,
	.sensor_v4l2_subdev_info = m9mo_iu081f2f_subdev_info,
	.sensor_v4l2_subdev_info_size = ARRAY_SIZE(m9mo_iu081f2f_subdev_info),
	.sensor_v4l2_subdev_ops = &m9mo_iu081f2f_subdev_ops,
	.func_tbl = &m9mo_iu081f2f_func_tbl,
};

module_init(m9mo_iu081f2f_init);
module_exit(m9mo_iu081f2f_exit);
