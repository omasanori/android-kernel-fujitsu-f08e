...
#define PLATFORM_DRIVER_NAME  "m7mo_iu134fbf"
#define m7mo_iu134fbf_obj m7mo_iu134fbf_##obj

#define LOGI(fmt, args...)      printk(KERN_INFO "m7mo_iu134fbf: " fmt, ##args)
//#define LOGI(fmt, args...)      do{}while(0)
//#define LOGD(fmt, args...)      printk(KERN_INFO "m7mo_iu134fbf: " fmt, ##args)
#define LOGD(fmt, args...)      do{}while(0)
#define LOGE(fmt, args...)      printk(KERN_ERR "m7mo_iu134fbf: " fmt, ##args)
...
/*===================================================================*
    LOCAL DECLARATIONS
 *===================================================================*/
DEFINE_MUTEX(m7mo_iu134fbf_mtx);
static struct msm_sensor_ctrl_t m7mo_iu134fbf_s_ctrl;
...
static void __i2c_err_dump(struct camsensor_i2c_command_isp* cmd)
{
	...
	if (cmd->rlen != 0) {
		...
		for (i=0; i<cmd->rlen; i++) {
			LOGE("<m7mo read err> [cate:0x%02x param:0x%02x]\n",
				cmd->category, cmd->byte+i);
		}
	} else {
		// write
		for (i=0; i<cmd->wlen; i++) {
			LOGE("<m7mo write err> [cate:0x%02x param:0x%02x] <-0x%02x\n",
...
static void __i2c_access_dump(struct camsensor_i2c_command_isp* cmd)
{
	...
	if (cmd->rlen != 0) {
		// read
		for (i=0; i<cmd->rlen; i++) {
			LOGD("<m7mo read ok> [cate:0x%02x param:0x%02x] ->0x%02x\n",
				cmd->category, cmd->byte+i, cmd->prdata[i]);
		}
	} else {
		// write
		for (i=0; i<cmd->wlen; i++) {
			LOGD("<m7mo write ok> [cate:0x%02x param:0x%02x] <-0x%02x\n",
				cmd->category, cmd->byte+i, cmd->pwdata[i]);
		}
	}
...
static int m7mo_iu134fbf_i2c_write_isp(
...
static int m7mo_iu134fbf_i2c_read_isp(
	struct i2c_client* client, struct camsensor_i2c_command_isp* cmd)
{
	int rc = 0;
	if (is_use_i2c_driver) {
...
static int m7mo_iu134fbf_i2c_write(
...
static int m7mo_iu134fbf_i2c_read(
	struct i2c_client* client, struct camsensor_i2c_command* cmd)
{
	int rc = 0;
	if (is_use_i2c_driver) {
...
static int32_t m7mo_iu134fbf_get_temp(struct msm_sensor_ctrl_t *s_ctrl,
			uint32_t* temp)
{
    int rc = 0;
...
static int32_t m7mo_iu134fbf_command(struct msm_sensor_ctrl_t *s_ctrl,
			struct cfg_cmd_ctrl_isp* cmd)
{
	...
	if (!cmd->rxlen) {
    	rc = m7mo_iu134fbf_i2c_write_isp(s_ctrl->sensor_i2c_client->client, &i2c_cmd);
	} else {
    	rc = m7mo_iu134fbf_i2c_read_isp(s_ctrl->sensor_i2c_client->client, &i2c_cmd);
		if (!rc) {
...
static int32_t m7mo_iu134fbf_command_normal(struct msm_sensor_ctrl_t *s_ctrl,
			struct cfg_cmd_ctrl* cmd)
{
	...
	if (!cmd->rxlen) {
    	rc = m7mo_iu134fbf_i2c_write(s_ctrl->sensor_i2c_client->client, &i2c_cmd);
	} else {
    	rc = m7mo_iu134fbf_i2c_read(s_ctrl->sensor_i2c_client->client, &i2c_cmd);
		if (!rc) {
			for (cpl=0; cpl<cmd->rxlen; cpl++) {
				cmd->rvalue[cpl] = i2c_cmd.prdata[cpl];
			}
...
static int32_t m7mo_iu134fbf_sensor_setting(struct msm_sensor_ctrl_t *s_ctrl,
			int update_type, int res)
{
    int32_t rc = 0;
//	uint32_t p_clk = 228570000;
//	uint32_t p_clk = 300000000;
	uint32_t p_clk = 259200000;
...
static int32_t m7mo_iu134fbf_set_sensor_mode(struct msm_sensor_ctrl_t *s_ctrl,
	int mode, int res)
{
    int32_t rc = 0;
    LOGI("+%s\n", __func__);
...
static int32_t m7mo_iu134fbf_sensor_mode_init(struct msm_sensor_ctrl_t *s_ctrl,
...
static int32_t m7mo_iu134fbf_sensor_config(struct msm_sensor_ctrl_t *s_ctrl,
	void __user *argp)
{
    ...
    case CFG_GET_TEMP:
        LOGD("-%s (CFG_SENSOR_INIT)!\n", __func__);
    	rc = m7mo_iu134fbf_get_temp(s_ctrl, &cfg.cfg.temp);
        if (rc < 0) {
    		break;
    	}
    ...
    case CFG_COMMAND:
        LOGD("-%s (CFG_COMMAND)\n", __func__);
    	rc = m7mo_iu134fbf_command(s_ctrl, &cfg.cfg.cmd_isp);
    	if (!rc) {
    ...
    case CFG_COMMAND_NORMAL:
    	...
    	rc = m7mo_iu134fbf_command_normal(s_ctrl, &cfg.cfg.cmd);
    	if (!rc) {
    		if (copy_to_user((void *)argp, &cfg, sizeof(struct sensor_cfg_data))) {
                rc = -EFAULT;
    		}
...
static struct bu1852gxw_int bu1852gxw_cam_gpios[] = {
	{"+EXP1_GPIO02", BU1852_EXP2_GPIO_TO_SYS(02), true, 1, 1},  //GPIO-EXP(ADR=1)-GPIO[02](ROW2) (+1.2V_M7MO_IST)
	{"+EXP1_GPIO01", BU1852_EXP1_GPIO_TO_SYS(01), true, 1, 1},  //GPIO-EXP(ADR=0)-GPIO[01](ROW1) (+1.8V_M7M0_EST)
	{"+EXP1_GPIO15", BU1852_EXP2_GPIO_TO_SYS(15), false, 1, 1}, //GPIO-EXP(ADR=1)–GPIO[15](COL7) (+1.2V_M7MO_CORE)
	{"+EXP1_GPIO19", BU1852_EXP1_GPIO_TO_SYS(19), false, 1, 2}, //GPIO-EXP(ADR=0)–GPIO[19](COL11) (+2.7V_OCAM)
	{"+EXP1_GPIO18", BU1852_EXP1_GPIO_TO_SYS(18), false, 1, 2}, //GPIO-EXP(ADR=0)–GPIO[18](COL10) (1.8V_OCAM)
	{"+EXP1_GPIO17", BU1852_EXP1_GPIO_TO_SYS(17), false, 1, 2}  //GPIO-EXP(ADR=0)–GPIO[17](COL9) (2.8V_OCAMA)
};
...
///////////
static int32_t m7mo_iu134fbf_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	int i,rc;
...
static int32_t m7mo_iu134fbf_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	int i;
	LOGI("+%s\n", __func__);
...
static const struct i2c_device_id m7mo_iu134fbf_i2c_id[] = {
	{PLATFORM_DRIVER_NAME, (kernel_ulong_t)&m7mo_iu134fbf_s_ctrl},
	{ },
};

int32_t m7mo_iu134fbf_i2c_probe(struct i2c_client *client,
...
#if 0
static int m7mo_resume(struct i2c_client *client)
{
	int i;
	LOGI("+%s\n", __func__);
...
static int m7mo_suspend(struct i2c_client *client, pm_message_t state)
{
	int i;
	LOGI("+%s\n", __func__);
...
static struct i2c_driver m7mo_iu134fbf_i2c_driver = {
	.id_table = m7mo_iu134fbf_i2c_id,
	.probe  = m7mo_iu134fbf_i2c_probe,
//	.suspend = m7mo_suspend,
//	.resume = m7mo_resume,
	.remove = __exit_p(m7mo_iu134fbf_i2c_remove),
	.driver = {
		.name = PLATFORM_DRIVER_NAME,
	},
};

static struct msm_camera_i2c_client m7mo_iu134fbf_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_BYTE_ADDR,
};

static const struct of_device_id m7mo_iu134fbf_dt_match[] = {
	{.compatible = "qcom,m7mo_iu134fbf", .data = &m7mo_iu134fbf_s_ctrl},
	{}
};

MODULE_DEVICE_TABLE(of, m7mo_iu134fbf_dt_match);

static struct platform_driver m7mo_iu134fbf_platform_driver = {
	.driver = {
		.name = "qcom,m7mo_iu134fbf",
		.owner = THIS_MODULE,
		.of_match_table = m7mo_iu134fbf_dt_match,
	},
};

static int32_t m7mo_iu134fbf_platform_probe(struct platform_device *pdev)
{
	int32_t rc = 0;
	const struct of_device_id *match;
	match = of_match_device(m7mo_iu134fbf_dt_match, &pdev->dev);
	rc = msm_sensor_platform_probe(pdev, match->data);
	return rc;
}

static int __init m7mo_iu134fbf_init(void)
{
	int rc = 0;
	int i = 0;
	LOGI("+%s\n", __func__);

	rc = platform_driver_probe(&m7mo_iu134fbf_platform_driver,
		m7mo_iu134fbf_platform_probe);
	if (!rc){
...
//		return rc;
	...
	if (is_use_i2c_driver) {
		rc = i2c_add_driver(&m7mo_iu134fbf_i2c_driver);
		if (rc < 0) {
			rc = -ENOTSUPP;
			LOGE("%s: I2C add driver failed\n", __func__);
			i2c_del_driver(&m7mo_iu134fbf_i2c_driver);
//			return rc;
		}
	}
	for (i = 0; i < BU1852GXW_CAM_GPIOS_VAL; i++) {
...
static void __exit m7mo_iu134fbf_exit(void)
{
	if (m7mo_iu134fbf_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&m7mo_iu134fbf_s_ctrl);
		platform_driver_unregister(&m7mo_iu134fbf_platform_driver);
	} else
		i2c_del_driver(&m7mo_iu134fbf_i2c_driver);
	return;
}

int32_t m7mo_iu134fbf_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	return 0; // nop
}

static struct msm_sensor_fn_t m7mo_iu134fbf_func_tbl = {
...
//	.sensor_write_snapshot_exp_gain = msm_sensor_write_exp_gain1,
	.sensor_setting = m7mo_iu134fbf_sensor_setting,
	.sensor_set_sensor_mode = m7mo_iu134fbf_set_sensor_mode,
	.sensor_mode_init = m7mo_iu134fbf_sensor_mode_init,
	.sensor_config = m7mo_iu134fbf_sensor_config,
	.sensor_power_up = m7mo_iu134fbf_sensor_power_up,
	.sensor_power_down = m7mo_iu134fbf_sensor_power_down,
	.sensor_get_csi_params = msm_sensor_get_csi_params,
	.sensor_match_id = m7mo_iu134fbf_sensor_match_id,
};

static struct msm_sensor_reg_t m7mo_iu134fbf_regs = {
	.default_data_type = MSM_CAMERA_I2C_BYTE_DATA,
//	.start_stream_conf = m7mo_iu134fbf_start_settings,
//	.start_stream_conf_size = ARRAY_SIZE(m7mo_iu134fbf_start_settings),
//	.stop_stream_conf = m7mo_iu134fbf_stop_settings,
//	.stop_stream_conf_size = ARRAY_SIZE(m7mo_iu134fbf_stop_settings),
//	.group_hold_on_conf = m7mo_iu134fbf_groupon_settings,
//	.group_hold_on_conf_size = ARRAY_SIZE(m7mo_iu134fbf_groupon_settings),
//	.group_hold_off_conf = m7mo_iu134fbf_groupoff_settings,
//	.group_hold_off_conf_size =
//		ARRAY_SIZE(m7mo_iu134fbf_groupoff_settings),
//	.init_settings = &m7mo_iu134fbf_init_conf[0],
//	.init_size = ARRAY_SIZE(m7mo_iu134fbf_init_conf),
//	.mode_settings = &m7mo_iu134fbf_confs[0],
//	.output_settings = &m7mo_iu134fbf_dimensions[0],
//	.num_conf = ARRAY_SIZE(m7mo_iu134fbf_confs),
};

static long m7mo_iu134fbf_sensor_subdev_ioctl(struct v4l2_subdev *sd,
...
//static struct v4l2_subdev_core_ops m7mo_iu134fbf_subdev_core_ops;
static struct v4l2_subdev_core_ops m7mo_iu134fbf_subdev_core_ops = {
	.ioctl = m7mo_iu134fbf_sensor_subdev_ioctl,
	.s_power = msm_sensor_power,
};

static struct v4l2_subdev_video_ops m7mo_iu134fbf_subdev_video_ops = {
	.enum_mbus_fmt = msm_sensor_v4l2_enum_fmt,
};

static struct v4l2_subdev_ops m7mo_iu134fbf_subdev_ops = {
	.core = &m7mo_iu134fbf_subdev_core_ops,
	.video  = &m7mo_iu134fbf_subdev_video_ops,
};

static struct v4l2_subdev_info m7mo_iu134fbf_subdev_info[] = {
	{
...
static struct msm_sensor_ctrl_t m7mo_iu134fbf_s_ctrl = {
	.msm_sensor_reg = &m7mo_iu134fbf_regs,
	.sensor_i2c_client = &m7mo_iu134fbf_sensor_i2c_client,
	.sensor_i2c_addr = 0x1F,
	.vreg_seq = NULL,
	.num_vreg_seq = 0,
//	.sensor_output_reg_addr = &m7mo_iu134fbf_reg_addr,
//	.sensor_id_info = &m7mo_iu134fbf_id_info,
//	.sensor_exp_gain_info = &m7mo_iu134fbf_exp_gain_info,
	.cam_mode = MSM_SENSOR_MODE_INVALID,
	.wait_num_frames = 0,
	.min_delay = 0,
	.msm_sensor_mutex = &m7mo_iu134fbf_mtx,
	.sensor_i2c_driver = &m7mo_iu134fbf_i2c_driver,
	.sensor_v4l2_subdev_info = m7mo_iu134fbf_subdev_info,
	.sensor_v4l2_subdev_info_size = ARRAY_SIZE(m7mo_iu134fbf_subdev_info),
	.sensor_v4l2_subdev_ops = &m7mo_iu134fbf_subdev_ops,
	.func_tbl = &m7mo_iu134fbf_func_tbl,
};

module_init(m7mo_iu134fbf_init);
module_exit(m7mo_iu134fbf_exit);
