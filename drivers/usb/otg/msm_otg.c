...
/* FUJITSU:2012-11-16 USB OMT add start */
#include "../arch/arm/mach-msm/board-8064.h"
/* FUJITSU:2012-11-16 USB OMT add end */

/* FUJITSU:2012-11-16 USB Add start */
...
/* FUJITSU:2012-11-16 USB Add end */
...
/* FUJITSU:2013-04-17 USB del start */
#if 0
...
/* FUJITSU:2012-11-16 USB start */
		msleep(100);
		gpio_set_value_cansleep(PM8921_GPIO_PM_TO_SYS(31), 0);
/* FUJITSU:2012-11-16 USB end */
...
/* FUJITSU:2013-02-13 mod start */
	if (mA!=0){
		dev_info(motg->phy.dev, "%s:CHG_types = %s\n", __func__,chg_to_string(motg->chg_type));
		if (motg->chg_type != USB_SDP_CHARGER){
			/* for USB_CHG_TYPE__WALLCHARGER */
			fj_chg_ac_vbus_draw(FJ_CHG_ENABLE);
			motg->cur_power = FJ_CHG_AC_CURRENT;
			dev_info(motg->phy.dev, "FJ_CHG_AC_CURRENT = %u\n", FJ_CHG_AC_CURRENT);
		}
		else
		{
			/* for USB_CHG_TYPE__SDP */
			fj_chg_usb_vbus_draw(FJ_CHG_ENABLE);
			motg->cur_power = FJ_CHG_USB_CURRENT;
			dev_info(motg->phy.dev, "FJ_CHG_USB_CURRENT = %u\n", FJ_CHG_USB_CURRENT);
		}
	}else{
		dev_info(motg->phy.dev, "%s:CHG_types = %s\n", __func__,chg_to_string(motg->chg_type));
		if (motg->chg_type != USB_SDP_CHARGER){
			/* for USB_CHG_TYPE__WALLCHARGER */
			fj_chg_ac_vbus_draw(FJ_CHG_DISABLE);
			motg->cur_power = FJ_CHG_OFF_CURRENT;
			dev_info(motg->phy.dev, "%s:FJ_CHG_AC_CURRENT = %u\n",__func__, FJ_CHG_OFF_CURRENT);
		}
		else
		{
			/* for USB_CHG_TYPE__SDP */
			fj_chg_usb_vbus_draw(FJ_CHG_DISABLE);
			motg->cur_power = FJ_CHG_OFF_CURRENT;
			dev_info(motg->phy.dev, "%s:FJ_CHG_USB_CURRENT = %u\n", __func__,FJ_CHG_OFF_CURRENT);
		}
	}
/* FUJITSU:2013-02-13 mod end */
...
/* FUJITSU:2012-11-16 USB del end */
	 ...
	 */
	if (on) {
/* FUJITSU:2013-04-17 USB mod start */
#if 0
...
#else
	gpio_set_value_cansleep(PM8921_GPIO_PM_TO_SYS(42), 1);
#endif
/* FUJITSU:2013-04-17 USB mod end */
...
/* FUJITSU:2013-04-17 USB mod start */
		...
		msm_otg_notify_host_mode(motg, on);
#else
	gpio_set_value_cansleep(PM8921_GPIO_PM_TO_SYS(42), 0);
#endif
/* FUJITSU:2013-04-17 USB mod end */
		vbus_is_on = false;
	}
...
#endif
/* FUJITSU:2013-04-17 USB del end */
		otg->phy->state = OTG_STATE_B_IDLE;
...
/* FUJITSU:2013-03-28 USB debugfs I/F add start */

static int msm_otg_ulpi_state_show(struct seq_file *s, void *unused)
{
	struct usb_phy *phy = &(the_msm_otg->phy);
	
	seq_printf(s, "ulpi_addr = 0x81, ulpi_data = 0x%x\n", ulpi_read(phy, 0x81));
	seq_printf(s, "ulpi_addr = 0x82, ulpi_data = 0x%x\n", ulpi_read(phy, 0x82));
	seq_printf(s, "ulpi_addr = 0x83, ulpi_data = 0x%x\n", ulpi_read(phy, 0x83));

	return 0;
}

static int msm_otg_ulpi_state_open(struct inode *inode, struct file *file)
{
	return single_open(file, msm_otg_ulpi_state_show, inode->i_private);
}

const struct file_operations msm_otg_ulpi_state_fops = {
	.open = msm_otg_ulpi_state_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
/* FUJITSU:2013-03-28 USB debugfs I/F add end */
...
/* FUJITSU:2012-12-18 USB debugfs I/F add start */
	msm_otg_dentry = debugfs_create_file("ulpi_state", S_IRUGO,
				msm_otg_dbg_root, motg, &msm_otg_ulpi_state_fops);

	if (!msm_otg_dentry) {
		debugfs_remove_recursive(msm_otg_dbg_root);
		return -ENODEV;
	}
/* FUJITSU:2012-12-18 USB debugfs I/F add end */
...
/* FUJITSU:2012-11-16 USB start */
	ovp_device_initialized(INITIALIZE_OTG);
/* FUJITSU:2012-11-16 USB end */
...
/* FUJITSU:2013-02-13 OVP mod start */
int msm_otg_enable_irq(int pType)
{
	struct msm_otg	*motg = the_msm_otg;
	struct usb_phy *phy = &motg->phy;

	dev_info(phy->dev, "******* msm_otg_enable_irq [%d]*******\n",pType);
	if (pType){
		msm_otg_set_vbus_state(1);
	}
	dev_info(phy->dev, "******* USB enabled *******\n");
	return 0;
}
EXPORT_SYMBOL(msm_otg_enable_irq);

int msm_otg_disable_irq(int pType)
{
	struct msm_otg	*motg = the_msm_otg;
	struct usb_phy *phy = &motg->phy;

	dev_info(phy->dev, "******* msm_otg_disable_irq [%d]*******\n",pType);
	if (pType){
		msm_otg_set_vbus_state(0);
	} else {
		gpio_set_value_cansleep(PM8921_GPIO_PM_TO_SYS(42), 0);
	}
	dev_info(phy->dev, "******* USB disabled *******\n");
	return 0;
}
EXPORT_SYMBOL(msm_otg_disable_irq); 
/* FUJITSU:2013-02-13 OVP mod end */
