...
/* FUJITSU:2012-11-16 OMT start */
#define CONFIG_USB_ANDROID_MMP y
/* FUJITSU:2012-11-16 OMT end */

/* FUJITSU:2013-01-16 USB OM add start */
#define CONFIG_USB_ANDROID_OM
#include <asm/system_info.h>
/* FUJITSU:2013-01-16 USB OM add end */
/*
...
extern bool debugusb_is_om(unsigned int pid);
...
{
/* FUJITSU:2012-11-16 USB start */
	if (android_get_function_enable(f, "adb"))
...
/* FUJITSU:2012-11-16 USB OMT start */
#ifdef CONFIG_USB_ANDROID_MMP
/* MMP */
...
#endif
/* FUJITSU:2012-11-16 USB OMT end */
...
static struct android_usb_function *supported_functions_fjdev[] = {
	&mbim_function,
	&ecm_qc_function,
	&rmnet_smd_function,
	&rmnet_sdio_function,
...
/* FUJITSU:2013-05-17 USB rndis del start */
static struct android_usb_function *supported_functions_fjdev005[] = {
	&mbim_function,
	&ecm_qc_function,
	&rmnet_smd_function,
	&rmnet_sdio_function,
...
/* FUJITSU:2013-05-17 USB rndis del end */
...
/* FUJITSU:FUJITSU:2012-11-16 USB start */
	pr_info("%s: buf=%s\n", __func__, buf);
/* FUJITSU:FUJITSU:2012-11-16 USB end */
	while (b) {
		conf_str = strsep(&b, ":");
		if (conf_str) {
...
/* FUJITSU:FUJITSU:2012-11-16 USB end */
	if (enabled && !dev->enabled) {
		 ...
		 */
/* FUJITSU:2013-01-16 USB mod start */
		if (device_desc.idProduct == 0) {
			pr_info("%s: this port is not valid, pid =[%x]\n",
											__func__, device_desc.idProduct);
			mutex_unlock(&dev->mutex);
			return size;
		}

		if (debugusb_is_om(device_desc.idProduct)) {
...
/* FUJITSU:2013-05-17 USB mod start */
#if 0
	android_dev->functions = supported_functions;
#else
	if ((system_rev & 0x000000F0) == 0x00000050 ||
		(system_rev & 0x000000F0) == 0x00000040) {
		android_dev->functions = supported_functions_fjdev;
	} else if ((system_rev & 0x000000F0) == 0x00000010) {
		android_dev->functions = supported_functions_fjdev005;
	} else {
		android_dev->functions = supported_functions;
	}
#endif
/* FUJITSU:2013-05-17 USB mod end */
