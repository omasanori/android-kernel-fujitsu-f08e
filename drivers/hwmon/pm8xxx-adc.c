...
#include <linux/gpio.h> /* FUJITSU:2012-11-20 HWMON add */
#include <asm/system_info.h>    /* FUJITSU:2013-04-11 HWMON add */
...
/* FUJITSU:2012-11-20 HWMON add start */
#define TEMP_FETON_GPIO	18				/* GPIO Number 18 */
#define PM8921_GPIO_PM_TO_SYS(pm_gpio)	(pm_gpio - 1 + NR_GPIO_IRQS)

#define CHG_CNTRL					0x204
#define VREF_BATT_THERM_FORCE_ON	BIT(7)

#define CHG_BMS_CTRL				0x224
/* FUJITSU:2012-11-20 HWMON add end */
...
/* FUJITSU:2013-02-21 HWMON add start */
/* MPP Configuration custom settings */
static struct pm8xxx_mpp_config_data pm8xxx_adc_mpp_config2 = {
    .type       = PM8XXX_MPP_TYPE_A_INPUT,
    .level      = PM8XXX_MPP_AIN_AMUX_CH7,
    .control    = PM8XXX_MPP_AOUT_CTRL_DISABLE,
};
/* FUJITSU:2013-02-21 HWMON add end */
...
static int32_t pm8xxx_adc_patherm_power(bool on)
{
	...
	if (on) {
		...
		if (rc < 0) {
			...
			/* FUJITSU:2012-11-20 HWMON add start */
			gpio_set_value_cansleep(PM8921_GPIO_PM_TO_SYS(TEMP_FETON_GPIO), 1);
			/* FUJITSU:2012-11-20 HWMON add end */
			return rc;
		}
		...
		/* FUJITSU:2012-11-20 HWMON add start */
		gpio_set_value_cansleep(PM8921_GPIO_PM_TO_SYS(TEMP_FETON_GPIO), 0);
		mdelay(2);
		/* FUJITSU:2012-11-20 HWMON add end */
	} else {
...
/* FUJITSU:2012-11-20 HWMON mod start */
#if 0
	switch (channel) {
...
#endif
/* FUJITSU:2012-11-20 HWMON mod end */
...
/* FUJITSU:2012-11-20 HWMON add start */
static int32_t pm8xxx_adc_masked_write_reg(uint32_t addr, u8 mask, u8 data)
{
	struct pm8xxx_adc *adc_pmic = pmic_adc;
	int rc;
	u8 reg;

	rc = pm8xxx_readb(adc_pmic->dev->parent, addr, &reg);
	if (rc) {
		pr_err("PM8xxx adc read reg %d failed with %d\n", addr, rc);
		return rc;
	}
	reg &= ~mask;
	reg |= data & mask;
	rc = pm8xxx_writeb(adc_pmic->dev->parent, addr, reg);
	if (rc) {
		pr_err("PM8xxx adc write reg %d failed with %d\n", addr, rc);
		return rc;
	}
			...
			struct pm8xxx_adc_chan_result *result)
{
	...
	/* FUJITSU:2012-11-20 HWMON add start */
	if (mpp_num == PM8XXX_AMUX_MPP_4) {
		rc = pm8xxx_adc_masked_write_reg(CHG_CNTRL, VREF_BATT_THERM_FORCE_ON, VREF_BATT_THERM_FORCE_ON);
		if (rc < 0) {
			pr_err("pm8xxx adc FET ON error with %d\n", rc);
			goto fail;
		}
	}
	/* FUJITSU:2012-11-20 HWMON add end */
    /* FUJITSU:2013-04-11 HWMON mod start */
    if ((system_rev & 0xF0) == 0x40) {
        if ((mpp_num == PM8XXX_AMUX_MPP_7) ||
            (mpp_num == PM8XXX_AMUX_MPP_9) ||
            (mpp_num == PM8XXX_AMUX_MPP_10)) {
            rc = pm8xxx_mpp_config(((mpp_num - 1) + adc_pmic->mpp_base),
                            &pm8xxx_adc_mpp_config2);
        } else {
            rc = pm8xxx_mpp_config(((mpp_num - 1) + adc_pmic->mpp_base),
    ...
    /* FUJITSU:2013-04-11 HWMON mod end */
	...
	/* FUJITSU:2012-11-20 HWMON add start */
	if (mpp_num == PM8XXX_AMUX_MPP_4) {
		rc = pm8xxx_adc_masked_write_reg(CHG_CNTRL, VREF_BATT_THERM_FORCE_ON, 0);
		if (rc < 0) {
			pr_err("pm8xxx adc FET OFF error with %d\n", rc);
			goto fail;
		}
	}
	/* FUJITSU:2012-11-20 HWMON add end */
...
/* FUJITSU:2013-03-01 HWMON add start */
	rc = pm8xxx_adc_masked_write_reg(CHG_BMS_CTRL, 0xFF, 0x00);
	if (rc < 0) {
		pr_err("pm8xxx BMS OFF error with %d\n", rc);
	}
/* FUJITSU:2013-03-01 HWMON add end */
